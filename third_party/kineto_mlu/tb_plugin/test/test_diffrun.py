...
class TestDiffRun(unittest.TestCase):
    ...
    @pytest.mark.skipif(not (os.path.isfile(os.path.expanduser('../samples/resnet50_mlu/worker0.1710905234237883642.pt.trace.json')) and
                        os.path.isfile(os.path.expanduser('../samples/resnet50_mlu/worker0.1710905235954230517.pt.trace.json'))),
                        reason="file doesn't exist")
    def test_happy_path(self):
        path1 = '../samples/resnet50_mlu/worker0.1710905234237883642.pt.trace.json'
        profile1 = load_profile('worker0', 1, path1)
        roots = list(profile1.tid2tree.values())
        root = roots[0]

        path2 = '../samples/resnet50_mlu/worker0.1710905235954230517.pt.trace.json'
