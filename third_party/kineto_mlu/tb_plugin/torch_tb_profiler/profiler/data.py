...
class RunProfileData:
    def __init__(self, worker: str, span: str, trace_json: Dict):
        ...
        for data in trace_body:
            # Currently, "fwdbwd" is for GPU, and "forward_backward" is for MLU
            if data.get('cat') == 'fwdbwd' or data.get('cat') == 'forward_backward':
        ...
        # TODO: Remove this after the bug is solved(see https://github.com/pytorch/kineto/issues/843).
        # Cause GPU trace file has a bug: the external id of RuntimeEvent and KernelEvent are wrong.
        # At present, use time interval containing relationship to build father-child correlation
        # for RuntimeEvent, which is also used for OperatorEvent. It exists bugs when RuntimeEvent
        # has duration 0 and starts at same time as a OperatorEvent.
        if self.device_type == 'gpu':
            self._correct_rt_external_id_for_gpu()
    ...
    def _correct_rt_external_id_for_gpu(self):
        for event in self.events:
            if event.type == EventTypes.RUNTIME:
                for evt in self.events:
                    if evt.ts > event.ts:
                        break
                    if evt.pid != event.pid or evt.tid != event.tid:
                        continue
                    if evt.type in [EventTypes.MEMORY, EventTypes.KERNEL, EventTypes.MEMCPY, EventTypes.MEMSET]:
                        continue
                    elif evt.type == EventTypes.RUNTIME and evt.correlation_id == event.correlation_id:
                        continue
                    else:
                        evt_end, event_end = evt.ts + evt.duration, event.ts + event.duration
                        if evt.ts < event.ts:
                            if evt_end >= event_end:
                                if evt.external_id is not None and int(evt.external_id) != 0:
                                    event.external_id = evt.external_id
                        elif evt.ts == event.ts:
                            if evt_end > event_end:
                                if evt.external_id is not None and int(evt.external_id) != 0:
                                    event.external_id = evt.external_id
                            elif evt_end == event_end:
                                if evt.type == EventTypes.RUNTIME:
                                    if evt.correlation_id < event.correlation_id and evt.correlation_id != evt.external_id:
                                        event.external_id = evt.external_id
                                else:
                                    if evt.external_id is not None and int(evt.external_id) != 0:
                                        event.external_id = evt.external_id
