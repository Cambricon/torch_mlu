...
class GPUMetricsParser:
    ...
    def get_gpu_metrics(self, device_type='gpu'):
        def build_trace_counter_gpu_util(gpu_id, start_time, counter_value):
            util_json = ("{{\"ph\":\"C\", \"name\":\"{} {} Utilization\", \"pid\":{}, \"ts\":{}, "
                         "\"args\":{{\"{} Utilization\":{}}}}}").format(device_type.upper(), gpu_id, gpu_id, start_time, device_type.upper(), counter_value)
            return util_json

        def build_trace_counter_sm_efficiency(gpu_id, start_time, counter_value):
            util_json = ("{{\"ph\":\"C\", \"name\":\"{} {} Est. SM Efficiency\", \"pid\":{}, \"ts\":{}, "
                         "\"args\":{{\"Est. SM Efficiency\":{}}}}}").format(device_type.upper(), gpu_id, gpu_id, start_time, counter_value)
            return util_json
    ...
    def get_gpu_metrics_data_tooltip(
        ...
        def process_gpu(gpu_id: int):
            ...
            gpu_metrics_data.append({'title': '{} {}:'.format(device_type.upper(), gpu_id), 'value': ''})
            ...
            if gpu_info is not None:
                ...
                pass
            gpu_metrics_data.append({'title': '{} Utilization'.format(device_type.upper()), 'value': '{} %'.format(
        ...
        tooltip_summary = 'The {} usage metrics:\n'.format(device_type.upper())
        tooltip = '{}\n{}'.format(tooltip_summary,  consts.TOOLTIP_GPU_UTIL.replace('GPU', device_type.upper()))
        if device_type == 'gpu':
