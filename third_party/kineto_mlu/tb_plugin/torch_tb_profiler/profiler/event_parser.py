...
class StepParser:
    ...
    def _parse_step(self, event: DurationEvent, comm_nodes: Dict[int, CommunicationNode]):
        ...
        dur = event.duration
        evt_type = event.type
        # MLU communication kernel maybe MEMCPY P2P
        if evt_type == EventTypes.KERNEL or evt_type == EventTypes.MEMCPY:
            if event.external_id in comm_nodes:
                self.role_ranges[ProfileRole.Communication].append((ts, ts + dur))
            elif evt_type == EventTypes.KERNEL:
        ...
        if evt_type in [EventTypes.PYTHON, EventTypes.OPERATOR,
                        EventTypes.PROFILER_STEP, EventTypes.USER_ANNOTATION]:
