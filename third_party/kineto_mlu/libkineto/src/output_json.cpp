...
void ChromeTraceLogger::handleActivity(
  ...
  // When some RUNTIMEs or KERNELs are not associated
  // with any cpu op, we set it's external id to 0. To avoid
  // incorrect association if the correlationId of a RUNTIME or KERNEL
  // is equal to the external id of other CPU ops.
  int external_id = 0;
