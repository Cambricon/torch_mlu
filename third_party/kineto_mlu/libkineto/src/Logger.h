...
#define LOG_IS_ON(severity) \
  (severity >= libkineto_mlu::Logger::severityLevel())

#define LOG_IF(severity, condition) \
  !(LOG_IS_ON(severity) && (condition)) ? (void)0 : libkineto_mlu::VoidLogger() & \
    libkineto_mlu::Logger(severity, __LINE__, __FILE__).stream()
...
#define FILENAME_HASH                             \
  __to_constant__<libkineto_mlu::Logger::hash( \
      libkineto_mlu::Logger::basename(__FILE__))>::val
#define VLOG_IS_ON(verbosity)                           \
  (libkineto_mlu::Logger::verboseLogLevel() >= verbosity && \
   (libkineto_mlu::Logger::verboseLogModules() & FILENAME_HASH) == FILENAME_HASH)
...
#define PLOG(severity) \
  libkineto_mlu::Logger(severity, __LINE__, __FILE__, errno).stream()

#define SET_LOG_SEVERITY_LEVEL(level) \
  libkineto_mlu::Logger::setSeverityLevel(level)

#define SET_LOG_VERBOSITY_LEVEL(level, modules)   \
  libkineto_mlu::Logger::setVerboseLogLevel(level); \
  libkineto_mlu::Logger::setVerboseLogModules(modules)

// Logging the set of devices the trace is collect on.
#define LOGGER_OBSERVER_ADD_DEVICE(device_count) \
  libkineto_mlu::Logger::addLoggerObserverDevice(device_count)

// Incrementing the number of events collected by this trace.
#define LOGGER_OBSERVER_ADD_EVENT_COUNT(count) \
  libkineto_mlu::Logger::addLoggerObserverEventCount(count)

// Record duration of trace in milliseconds.
#define LOGGER_OBSERVER_SET_TRACE_DURATION_MS(duration) \
  libkineto_mlu::Logger::setLoggerObserverTraceDurationMS(duration)

// Record the trace id when given.
#define LOGGER_OBSERVER_SET_TRACE_ID(tid) \
  libkineto_mlu::Logger::setLoggerObserverTraceID(tid)

// Record the group trace id when given.
#define LOGGER_OBSERVER_SET_GROUP_TRACE_ID(gtid) \
  libkineto_mlu::Logger::setLoggerObserverGroupTraceID(gtid)

// Log the set of destinations the trace is sent to.
#define LOGGER_OBSERVER_ADD_DESTINATION(dest) \
  libkineto_mlu::Logger::addLoggerObserverDestination(dest)

// Record this was triggered by On-Demand.
#define LOGGER_OBSERVER_SET_TRIGGER_ON_DEMAND() \
  libkineto_mlu::Logger::setLoggerObserverOnDemand()

// Record this was triggered by On-Demand.
#define LOGGER_OBSERVER_ADD_METADATA(key, value) \
  libkineto_mlu::Logger::addLoggerObserverAddMetadata(key, value)


// UST Logger Semantics to describe when a stage is complete.
#define UST_LOGGER_MARK_COMPLETED(stage) \
  LOG(libkineto_mlu::LoggerOutputType::STAGE) << "Completed Stage: " << stage
