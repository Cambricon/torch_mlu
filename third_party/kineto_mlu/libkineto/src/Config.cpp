...
constexpr int kDefaultActivitiesMaxMluBufferSize(128 * 1024 * 1024);
...
constexpr int kDefaultMaxEventProfilersPerMlu(1);
...
constexpr char kMaxEventProfilersPerMluKey[] = "MAX_EVENT_PROFILERS_PER_MLU";
...
constexpr char kActivitiesMaxMluBufferSizeKey[] =
    "ACTIVITIES_MAX_MLU_BUFFER_SIZE_MB";
...
Config::Config()
    : verboseLogLevel_(-1),
      ...
      eventProfilerMaxInstancesPerMlu_(kDefaultMaxEventProfilersPerMlu),
      ...
      activitiesMaxMluBufferSize_(kDefaultActivitiesMaxMluBufferSize),
...
bool Config::handleOption(const std::string& name, std::string& val) {
  ...
  } else if (!name.compare(kMaxEventProfilersPerMluKey)) {
    eventProfilerMaxInstancesPerMlu_ = toInt32(val);
  ...
  // Activity Profiler
    ...
    activitiesOnDemandTimestamp_ = timestamp();
  } else if (!name.compare(kActivitiesMaxMluBufferSizeKey)) {
    activitiesMaxMluBufferSize_ = toInt32(val) * 1024 * 1024;
...
void Config::printActivityProfilerConfig(std::ostream& s) const {
  ...
  s << "Max MLU buffer size: " << activitiesMaxMluBufferSize() / 1024 / 1024
    << "MB" << std::endl;
