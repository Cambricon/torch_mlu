...
def _is_supported_device(tensor: torch.Tensor) -> bool:
    return tensor.is_mlu or tensor.device.type in ("cpu",)
    ...
    * Sync inf/nan for scaled gradient tensors on any torch.device (where tensors are placed) across
    ...
    def unscale_(self, optimizer: torch.optim.Optimizer) -> None:
        ...
        for found_inf in optimizer_state["found_inf_per_device"].values():
            if self._device == "mlu" and found_inf.device.type == "cpu":
                found_inf_on_cpus.append(found_inf)
                found_inf_on_mlu = found_inf.mlu()
                found_inf_on_mlus.append(found_inf_on_mlu)
                works.append(
                    dist.all_reduce(
                        found_inf_on_mlu, async_op=True, group=self.process_group
                    )
                )
            else:
                works.append(
        ...
        if found_inf_on_cpus:
            # TODO: use _foreach_copy_ when we support foreach kernel
            # torch._foreach_copy_(found_inf_on_cpus, found_inf_on_mlus)
            for found_inf_on_cpu, found_inf_on_mlu in zip(
                found_inf_on_cpus, found_inf_on_mlus
            ):
                found_inf_on_cpu.copy_(found_inf_on_mlu)
    ...
    def update(self, new_scale: Optional[Union[float, torch.Tensor]] = None) -> None:
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = "new_scale should be a float or a 1-element torch.mlu.FloatTensor or \
