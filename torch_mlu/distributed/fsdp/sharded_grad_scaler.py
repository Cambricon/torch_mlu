...
from torch.mlu import FloatTensor  # type: ignore[attr-defined]
from torch.mlu.amp.grad_scaler import _MultiDeviceReplicator, GradScaler, OptState
        ...
        for v in optimizer_state["found_inf_per_device"].values():
            if v.device.type == "cpu":
                ...
                future_handles.append(
                    dist.all_reduce(
                        v_on_mlu, async_op=True, group=self.process_group
                    ).get_future()
                )
                v.copy_(v_on_mlu.cpu())
            else:
                future_handles.append(
                    dist.all_reduce(
