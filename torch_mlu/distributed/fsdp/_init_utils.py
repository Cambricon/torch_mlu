...
# For custom backend, user must pass device_id if model is on CPU, otherwise a CUDA
# not compiled error will be raised because Pytorch set CUDA as default device.
# In this case, FSDP will be constructed on CPU for CUDA. However, for custom device,
# if you pass device_id, FSDP will be constructed on custom device. The different behavior
# may influence some strategies. So we change default device to MLU.
@no_type_check
def _init_device_handle(
    ...
    If the module is on CPU or meta, then the device type is the current mlu device.
...
def _get_device_from_device_id(
    ...
    if device == torch.device("mlu"):
        warnings.warn(
            f"FSDP got the argument `device_id` {device_id} on rank "
            f"{rank}, which does not have an explicit index. "
            f"FSDP will use the current device {torch.mlu.current_device()}. "
            "If this is incorrect, please explicitly call `torch.mlu.set_device()` "
...
def _get_compute_device(
    ...
    always a MLU device with its explicit index.
    ...
    if param is not None and param.device.type != "cpu":
        ...
        if device_from_device_id is not None and device_from_device_id.type != "mlu":
            compute_device = device_from_device_id  # Determined by custom backend
        else:
            compute_device = torch.device("mlu", torch.mlu.current_device())
...
# monkey-patch torch.distributed.fsdp._init_utils._init_device_handle
# directly may bring error due to import order, so we replace the __code__.
def apply_init_utils_patch():
    torch.distributed.fsdp._init_utils._init_device_handle.__code__ = (
        _init_device_handle.__code__
    )
