...
typedef ska::flat_hash_map<void*, std::unique_ptr<char[]>> PtrMap;
...
static PyObject* THMPStorage_shareMlu(PyObject* Module, PyObject* args) {
  ...
  if (storage.data()) {
    ...
    if (it == ptrMap.end()) {
      ...
      _handle =
          PyBytes_FromStringAndSize((char*)&handle, sizeof(cnrtIpcMemHandle));
      auto buffer = std::make_unique<char[]>(sizeof(cnrtIpcMemHandle));
      std::memcpy(buffer.get(), (char*)&handle, sizeof(cnrtIpcMemHandle));
      ptrMap.insert({base_ptr, std::move(buffer)});
    } else {
      _handle =
          PyBytes_FromStringAndSize(it->second.get(), sizeof(cnrtIpcMemHandle));
    }
    lock.unlock();
