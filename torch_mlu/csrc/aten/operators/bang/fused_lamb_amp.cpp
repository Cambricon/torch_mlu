...
void _fused_lamb_amp_common(
  ...
  int* noop_flag_ptr =
      static_cast<int*>(mlu_data_ptr(getMluTensorImpl(noop_flag)));
  float* lr_ptr =
      static_cast<float*>(mlu_data_ptr(getMluTensorImpl(learning_rate)));
  int* step_ptr = static_cast<int*>(mlu_data_ptr(getMluTensorImpl(step)));
  float* ggn_ptr =
      static_cast<float*>(mlu_data_ptr(getMluTensorImpl(global_grad_norm)));
  float* inv_scale_ptr =
      static_cast<float*>(mlu_data_ptr(getMluTensorImpl(inv_scale)));
  float* max_grad_norm_ptr =
      static_cast<float*>(mlu_data_ptr(getMluTensorImpl(max_grad_norm)));
  ...
  for (size_t tensor_id = 0; tensor_id < tensor_num; ++tensor_id) {
    ...
    if (is_amp) {
      ...
      half_p.addresses[tensor_count] =
          mlu_data_ptr(getMluTensorImpl(dst_param_contiguous));
    }
