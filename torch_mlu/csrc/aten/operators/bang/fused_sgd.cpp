...
bool bang_fused_sgd(
    ...
    const c10::List<c10::optional<at::Tensor>>& params_out,
    double weight_decay,
    double momentum,
    double dampening,
  ...
  for (int tensor_id = 0; tensor_id < tensor_num; ++tensor_id) {
    ...
    auto in_ptr = getMluTensorImpl(in_contiguous)->mlu_data_ptr();
    auto mt_ptr = getMluTensorImpl(mt_contiguous)->mlu_data_ptr();
    ...
    if (N == 4) {
      ...
      auto out_ptr = getMluTensorImpl(out_contiguous)->mlu_data_ptr();
