...
std::tuple<at::Tensor, at::Tensor> cnnl_prelu_backward_internal(
  ...
  // malloc mlu memory
  auto input_ptr = getMluTensorImpl(self)->mlu_data_ptr();
  auto grad_ptr = getMluTensorImpl(grad)->mlu_data_ptr();
  auto weight_ptr = getMluTensorImpl(weight)->mlu_data_ptr();
  auto grad_input_ptr = getMluTensorImpl(grad_input)->mlu_data_ptr();
  auto grad_weight_ptr = getMluTensorImpl(grad_weight)->mlu_data_ptr();
  // set descriptor config
