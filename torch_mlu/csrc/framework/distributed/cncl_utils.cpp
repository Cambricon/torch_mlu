...
void gather(
  ...
  auto input_ptr = mlu_data_ptr(torch_mlu::getMluTensorImpl(inputs));
  ...
  if (cur_rank == root) {
    for (const auto r : c10::irange(numranks)) {
      if (r != root) {
        auto output_ptr = mlu_data_ptr(torch_mlu::getMluTensorImpl(outputs[r]));
  ...
  if (cur_rank == root) {
    for (const auto r : c10::irange(numranks)) {
      if (r != root) {
        ...
        auto send_ptr = mlu_data_ptr(torch_mlu::getMluTensorImpl(inputs[r]));
    ...
    auto recv_ptr = mlu_data_ptr(torch_mlu::getMluTensorImpl(outputs));
...
void DebugInfoWriter::write(const std::string& cnclTrace) {
  ...
  if (!file.is_open()) {
    LOG(ERROR) << "Error opening file for writing CNCLPG debug info: "
               << filename_;
    return;
    ;
  }
  file.write(cnclTrace.data(), cnclTrace.size());
  LOG(INFO) << "Finished writing CNCLPG debug info to " << filename_;
}

DebugInfoWriter& DebugInfoWriter::getWriter(int rank) {
  if (writer_ == nullptr) {
    // support read NCCL environment
    std::string fileNamePrefix = getCvarString(
        {"TORCH_CNCL_DEBUG_INFO_TEMP_FILE", "TORCH_NCCL_DEBUG_INFO_TEMP_FILE"},
        "/tmp/cncl_trace_rank_");
...
float getDurationFromEvent(
  ...
  return cnclStartEvent.elapsed_time(cnclEndEvent);
}
