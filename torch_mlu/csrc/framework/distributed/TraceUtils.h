...
/* Trace Utils Related to TORCH_CNCL_DESYNC_DEBUG */
...
inline bool traceUpdate(
  ...
  try {
    ...
    LOG(ERROR) << "c10d::Store is down while updating #" << seq << " with key "
               << key;
    return false;
  }
  return true;
...
inline bool parseTraceValue(
  ...
  try {
    ...
    LOG(ERROR) << "c10d::Store is down while getting key " << key;
    return false;
  }
  return true;
}
