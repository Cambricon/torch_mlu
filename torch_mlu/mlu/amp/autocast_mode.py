...
class autocast(torch.amp.autocast_mode.autocast):
    ...
    ``torch.mlu.amp.autocast(args...)`` is equivalent to ``torch.autocast("mlu", args...)``
    """
...
# which may be falsely detected as "Iterables."
...
def custom_fwd(fwd=None, *, cast_inputs=None):
    ...
    Args:
        cast_inputs (:class:`torch.dtype` or None, optional, default=None):  If not ``None``,
            ...
            floating-point MLU Tensors to the target dtype (non-floating-point Tensors are not affected),
    ...
    def decorate_fwd(*args, **kwargs):
        args[0]._dtype = torch.get_autocast_dtype("mlu")
        if cast_inputs is None:
            args[0]._fwd_used_autocast = torch.is_autocast_enabled("mlu")
            return fwd(*args, **kwargs)
        else:
            autocast_context = torch.is_autocast_enabled("mlu")
