...
class FusedSGD(Optimizer):
    ...
    :class:`torch_mlu.optimizers.FusedSGD` may be used as a drop-in replacement for ``torch.optim.SGD``::

        opt = torch_mlu.optimizers.FusedSGD(model.parameters(), lr = ....)
        ...
        opt.step()
    ...
    Args:
        params (iterable): iterable of parameters to optimizer or dicts defining
            parameter groups
    ...
    def step(self, closure=None):
        ...
        for gid, group in enumerate(self.param_groups):
            ...
            # For each group, there are 4 possible combinations we need to consider:
            # grad_type, param_to_update_type, momentum_type, requires_fp16_model_copy
            # 1. fp16, fp16, fp16, No
            # 2. fp32, fp32, fp32, No
            # 3. fp16, fp32, fp32, Yes
            # 4. fp32, fp32, fp32, Yes
