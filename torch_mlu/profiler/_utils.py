...
class BasicEvaluation:
    ...
    def compute_queue_depth(self):
        ...
        mlu_kernel_events = sorted(
            (e for e in mlu_event_list if is_mlu_kernel(e)), key=lambda x: x.start_ns()
        )

        self.mlu_events = sorted(
            mlu_launch_events + mlu_kernel_events, key=lambda x: x.start_ns()
        )
        ...
        for event in all_events:
            ...
            while (
                current_kernel_index < len(mlu_kernel_events)
                and (mlu_kernel_events[current_kernel_index].start_ns()) <= start_time
            ):
