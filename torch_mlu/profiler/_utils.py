...
class BasicEvaluation:
    ...
    def compute_queue_depth(self):
        ...
        mlu_kernel_events = sorted(
            (e for e in mlu_event_list if is_mlu_kernel(e)), key=lambda x: x.start_us()
        )

        self.mlu_events = sorted(
            mlu_launch_events + mlu_kernel_events, key=lambda x: x.start_us()
        )
        ...
        for event in all_events:
            ...
            while (
                ...
                and (mlu_kernel_events[current_kernel_index].start_us()) * 1000
                <= start_time
            ):
