import itertools
import torch
from torch.autograd import DeviceType, ProfilerActivity
...
class EventList(list):
    ...
    def __init__(self, *args, **kwargs):
        use_mlu = kwargs.pop("use_cuda", True)
    ...
    def table(
        ...
        Args:
            sort_by (str, optional): Attribute used to sort entries. By default
                ...
                Valid keys include: ``cpu_time``, ``mlu_time``, ``cpu_time_total``,
                ``mlu_time_total``, ``cpu_memory_usage``, ``mlu_memory_usage``,
                ``self_cpu_memory_usage``, ``self_mlu_memory_usage``, ``count``.
            top_level_events_only(bool, optional): Boolean flag to determine the
                selection of events to display. If true, the profiler will only
                display events at top level like top-level invocation of python
                `lstm`, python `add` or other functions, nested events like low-level
                cpu/mlu ops events are omitted for profiler result readability.
    ...
    def supported_export_stacks_metrics(self):
        return ["self_cpu_time_total", "self_mlu_time_total"]
...
class FormattedTimesMixin(object):
    ...
    mlu_time_str = _attr_formatter("mlu_time")
    cpu_time_total_str = _attr_formatter("cpu_time_total")
    mlu_time_total_str = _attr_formatter("mlu_time_total")
    self_cpu_time_total_str = _attr_formatter("self_cpu_time_total")
    self_mlu_time_total_str = _attr_formatter("self_mlu_time_total")
    ...
    @property
    def mlu_time(self):
        return 0.0 if self.count == 0 else 1.0 * self.mlu_time_total / self.count  # type: ignore[attr-defined]
...
class FunctionEvent(FormattedTimesMixin):
    ...
    def __init__(
        ...
        self.mlu_memory_usage: int = cuda_memory_usage
        ...
        if self.device_type == DeviceType.CPU:
            if not self.is_legacy:
                ...
                return sum(kinfo.duration for kinfo in self.kernels) + sum(
                    ch.mlu_time_total for ch in self.cpu_children
                )
            else:
    ...
    @property
    def self_mlu_time_total(self):
        if self.is_async:
            return 0
        if self.device_type == DeviceType.CPU:
            return self.mlu_time_total - sum(
                [child.mlu_time_total for child in self.cpu_children]
            )
        else:
            assert self.device_type == DeviceType.PrivateUse1
            return self.mlu_time_total
    ...
    def __repr__(self):
        return (
            ...
            "cpu_children={} mlu_time={} name={} thread={} input_shapes={} "
            "cpu_memory_usage={} mlu_memory_usage={} is_async={} is_remote={} seq_nr={} is_legacy={}>".format(
                self.id,
                self.name,
                self.device_type,
                self.node_id,
...
class FunctionEventAvg(FormattedTimesMixin):
    ...
    def __init__(self):
        ...
        self.mlu_time_total: int = 0
        self.self_cpu_time_total: int = 0
        self.self_mlu_time_total: int = 0
        self.input_shapes: Optional[List[List[int]]] = None
        self.stack: Optional[List] = None
        self.scope: Optional[int] = None
        self.cpu_memory_usage: int = 0
        self.mlu_memory_usage: int = 0
        self.self_cpu_memory_usage: int = 0
        self.self_mlu_memory_usage: int = 0
    ...
    def add(self, other):
        ...
        self.self_mlu_time_total += other.self_mlu_time_total
    ...
    def __repr__(self):
        return (
            ...
            " self_mlu_time={} mlu_time={} input_shapes={} "
            "cpu_memory_usage={} mlu_memory_usage={}>".format(
                self.key,
                self.self_cpu_time_total_str,
                self.cpu_time_str,
                self.self_mlu_time_total_str,
                self.mlu_time_str,
                str(self.input_shapes),
                self.cpu_memory_usage,
                self.mlu_memory_usage,
...
def _build_table(
    ...
    has_mlu_time = any([event.self_mlu_time_total > 0 for event in events])
    has_mlu_mem = any([event.self_mlu_memory_usage > 0 for event in events])
    ...
    sum_self_mlu_time_total = 0
    for evt in events:
        if evt.device_type == DeviceType.CPU:
            # in legacy profiler, kernel info is stored in cpu events
            if evt.is_legacy:
                sum_self_mlu_time_total += evt.self_mlu_time_total
        elif evt.device_type == DeviceType.PrivateUse1:
            # in kineto profiler, there're events with the correct device type (e.g. MLU)
            sum_self_mlu_time_total += evt.self_mlu_time_total
    ...
    for evt in events:
        ...
        if has_mlu_time:
            row_values.extend(
                [
                    evt.self_mlu_time_total_str,
                    # MLU time total %
                    _format_time_share(
                        evt.self_mlu_time_total, sum_self_mlu_time_total
                    ),
        ...
        if profile_memory:
            ...
            if has_mlu_mem:
                row_values.extend(
                    [
                        # MLU Mem Total
                        _format_memory(evt.mlu_memory_usage),
                        # Self MLU Mem Total
                        _format_memory(evt.self_mlu_memory_usage),
                    ]
                )
        row_values.append(
    ...
    if has_mlu_time:
        append("Self MLU time total: {}".format(_format_time(sum_self_mlu_time_total)))
    return "".join(result)
...
def supported_activities():
    ...
    """
    supported_activities = torch.autograd._supported_activities()
    supported_activities.add(ProfilerActivity.MLU)
