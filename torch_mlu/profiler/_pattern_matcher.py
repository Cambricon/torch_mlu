...
class ExtraMLUCopyPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.TENSOR_DOT_MLU_PATTERN = re.compile(
            r"^torch_mlu/__init__\.py\(\d+\): _mlu$"
        )
    ...
    def match(self, event):
        ...
        if event is None:
            return False
        # .mlu() call stack is:
        #  | torch_mlu/__init__.py(39): mlu
        #  | <built-in function mlu>
        #  | aten::to
        # One level deeper than the GPU call stack
        if self.TENSOR_DOT_MLU_PATTERN.search(event.parent.name):
            event = event.parent
    ...
    def benchmark(self, events: List[_ProfilerEvent]):
        ...
        for shape in shapes_factor_map:
            ...
            to_timer = benchmark.Timer(
                stmt='torch.ones(size).to("mlu")', globals={"size": size}
            )
            de_timer = benchmark.Timer(
                stmt='torch.ones(size, device="mlu")', globals={"size": size}
            )
...
class FP32MatMulPattern(Pattern):
    ...
    def skip(self):
        has_tf32 = (
            torch.mlu.get_device_properties(torch.mlu.current_device()).major >= 5
        )
    ...
    def match(self, event: _ProfilerEvent):
        ...
        if event.name == "aten::mm":
            if torch_mlu._MLUC._get_cnmatmul_allow_tf32() is False:
                return True
        return False
...
class MatMulDimInFP16Pattern(Pattern):
    ...
    def benchmark(self, events: List[_ProfilerEvent]):
        ...
        for shape in shapes_factor_map:
            ...
            matrixA = torch.randn(
                closest_multiple(shape[0], 8), device="mlu", dtype=torch.float16
            )
            matrixB = torch.randn(
                closest_multiple(shape[1], 8), device="mlu", dtype=torch.float16
            )
