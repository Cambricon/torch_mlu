        ...
        **kwargs
        ...
        if pmus:
            ...
            if device_id not in DeviceNode.frequency_map:
                DeviceNode.frequency_map[device_id] = device_prop.frequency
            if device_id not in DeviceNode.bandwidth_map:
                DeviceNode.bandwidth_map[device_id] = device_prop.dram_bandwidth
            if device_id not in DeviceNode.core_count_map:
        ...
        self.hit_rate = (
            pmus.get("llc__tagram_hit") / self.llc_total if self.llc_total else "N/A"
        )
