...
class MemoryRecord:
    ...
    def action_type(self):
        return "ALLOCATE" if self.is_allocation else "RELEASE"

    @property
    def full_op_name_or_unknow(self):
        name = self.op_name_or_unknown
        if name.startswith("aten::empty") and self.parent_op_name != "CallTreeRoot":
            # aten::empty can be treated as the 'malloc' in pytorch
            name = f"{self.parent_op_name} ({name})"
        return name
    ...
    def data(self):
        return [
            self.tid,
            self.full_op_name_or_unknow,
            self.ts,
            self.action_type,
            round(self.bytes / 1024.0, 3),
            round(self.total_allocated / 1024.0, 3),
            round(self.total_reserved / 1024.0, 3),
            self.addr,
            self.device_name,
        ]
...
class MemorySnapshot:
    def __init__(
        ...
        processed_nodes: Dict[OperatorNode, int],
    ) -> None:
    ...
    def get_peak_memory(self) -> Dict[Tuple[DeviceType, int], int]:
        ...
        for r in self.memory_records:
            if r.total_allocated == r.total_allocated:  # !isnan
                peaks[(r.device_type, r.device_id)] = max(
                    peaks[(r.device_type, r.device_id)], r.total_allocated
                )
        return peaks

    def get_memory_statistics(
        self, tid2tree: Dict[int, OperatorNode], start_ts=None, end_ts=None
        ...
        memory_metrics_keyed_by_node: Dict[
            OperatorNode, Dict[str, List[int]]
        ] = defaultdict(dict_factory)
            ...
            if node not in self.processed_node:
                ...
                for device, metrics in node_memory_metrics.items():
                    ...
                    for i, value in enumerate(metrics):
                        ...
                        memory_metrics_keyed_by_node[node][device][
                            i + self_metric_length
                        ] += value
            else:
                logger.debug(
                    "node {}:{} is not operator node, will skip its self metrics processing".format(
        ...
        memory_metrics_keyed_by_nodename: Dict[str, Dict[str, List[int]]] = defaultdict(
            dict_factory
        )
    ...
    def get_memory_metrics(self, op: OperatorNode, start_ts, end_ts):
        metrics_count = len(
            [e.name for e in MemoryMetrics if e.name.startswith("Self")]
        )
...
class MemoryParser:
    def __init__(self, memory_events: Iterable[MemoryEvent]):
        ...
        self.memory_records: List[MemoryRecord] = [
            MemoryRecord.from_event(e) for e in memory_events
        ]
    ...
    def find_memory_nodes(self, tid2tree: Dict[int, OperatorNode]) -> MemorySnapshot:
        ...
        for tid, records in records_by_tid.items():
            ...
            while record_index < len(records):
                ...
                if (
                    ...
                    or current_node.start_time is None
                    or current_node.end_time is None
                ):
                ...
                if record.ts < current_node.start_time:
                    ...
                    logger.debug(
                        "record timestamp %d is less that the start time of %s"
        ...
        if len(self.staled_records) > 0 and len(self.memory_records) > 0:
            logger.debug(
                "{} memory records are skipped in total {} memory records and only {} get processed".format(
                    len(self.staled_records),
                    len(self.memory_records),
                    len(self.processed_records),
                )
            )
        if tree_height > 0:
