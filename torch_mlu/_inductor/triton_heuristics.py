...
def _precompile_config(self, cfg: Config, warm_cache_only_with_cc: Optional[int]):
    ...
    compile_meta["is_linear"] = (
        is_linear_memory_enabled() if device_prop.supports_linear_memory else False
    )
    compile_meta["cc"] = cc
...
def pointwise(
    ...
    device_type = triton_meta["device_type"] if "device_type" in triton_meta else "mlu"
    mlu_device = get_interface_for_device(device_type)
    device_prop = mlu_device.Worker.get_device_properties(triton_meta["device"])
    core_num = device_prop.multi_processor_count
    num_warps = 1
