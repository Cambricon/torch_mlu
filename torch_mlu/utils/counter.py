...
def _check_gencase():
    ...
    start_count = os.environ.get("CATCH_COUNTER_START")
    end_count = os.environ.get("CATCH_COUNTER_END")
    gencase_level = os.environ.get("CATCH_COUNTER_LEVEL")
    ...
    if start_count is not None and end_count is not None and gencase_level is not None:
        if gencase_level not in gencase_level_list:
            warnings.warn(
                f"CATCH_COUNTER enable failed, CATCH_COUNTER_LEVEL only support "
...
def _update_and_check_for_gencase():
    ...
    if cur_count == _GENCASE_STATUS_DICT["start_count"]:
        ...
        print(f"final CATCH_COUNTER is {cur_count - 1}")
        _GENCASE_ENABLED = False
        dump_cnnl_gencase(0)
        return
    ...
    print(f"current CATCH_COUNTER is {cur_count - 1}")
    return
