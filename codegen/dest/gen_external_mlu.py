...
# NB: _copy_from_and_resize in native_functions.yaml is not marked device_guard False,
# but MLU does not need it.
...
class RegisterMLU:
    ...
    def gen_device_check(
        ...
        device_check = "c10::optional<c10::Device> common_device = c10::nullopt;\n"
...
const c10::DeviceGuard device_guard(device_or_default(device));"""
                    ...
                    device_guard = (
                        f"at::globalContext().lazyInitMLU();\n{device_guard}"
                    )
                else:
    ...
    def gen_class(
        ...
        *,
        ...
        if k is SchemaKind.functional:
            ...
            proxy_field = f"std::array<c10::optional<c10::ExclusivelyOwned<at::Tensor>>, {len(f.func.returns)}> proxy_outputs_;"
        elif k is SchemaKind.out:
            output_type = "std::reference_wrapper<at::Tensor>"
            output_value = "proxy_outputs_[output_idx].has_value() ? **proxy_outputs_[output_idx] : outputs_[output_idx].get()"
            proxy_field = f"std::array<c10::optional<c10::ExclusivelyOwned<at::Tensor>>, {len(f.func.returns)}> proxy_outputs_;"
...
"""
            ...
            tensor_bridge = f""
            if self.g.out.structured_inherits == "TensorIteratorBase":
                tensor_bridge = f"""TensorIteratorBridge iter;
iter.to_build(op, "{self.g.functional.func.name.name}");"""
            sig_body.append(f"""\
{tensor_bridge}
op.impl({impl_exprs});
""")
