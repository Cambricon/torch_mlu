...
def parse_mlu_aten_yaml(
    ...
    for op in supported_aten:
        ...
        # example1 -> parse to a dict
        # derived_type:
        #   PrivateUse1: cnnl
        #   SparsePrivateUse1: bang
        # example2 -> parse to a str
        # derived_type: bang
        if isinstance(d['derived_type'], dict):
            d['derived_type'].pop('__line__')
            for k, v in d['derived_type'].items():
                assert v in ['cnnl', 'bang', 'mluop'], \
                    f"derived_type support cnnl, bang or mluop, but got: {v}"
        else:
...
def get_dispatch_key_aux(
    ...
    for op, args in aux.items():
        ...
        if isinstance(aux[op]["derived_type"], dict):
            dis_aux[op]["derived_type"] = aux[op]["derived_type"].get(str(dispatch_key), 'cnnl')
    return dis_aux
...
def run(options) -> None:
    ...
    mlu_aux = get_dispatch_key_aux(aux, DispatchKey.PrivateUse1)
        ...
        fm.write_with_template(
            ...
            'KernelFunctions.h', lambda: {
                ...
                'dispatch_kernel_declarations': list(concatMap(
                    GenExternalMLU(target, selector, mlu_aux), functions
                )) + list(concatMap(
