...
PYTORCH_VERSION="2.3.0" # set to native release version(eg: 2.1.0, 2.2.0) or branch(eg: release/2.1, release/2.2) in torch_mlu develop branch
TORCH_MLU_BRANCH='r2.3_develop'
VISION_VERSION='0.18.0'  # set to native release version(eg: 1.6.0) in torch_mlu develop branch
AUDIO_VERSION='2.3.0'  # set to native release version(eg: 2.1.0) in torch_mlu develop branch
...
fetch_neuware_ver_func() {
  ...
  if [ $GENESIS_TAG != "release" ] ; then
      ...
      GENESIS_VERSION=$(echo $GENESIS_VERSION | cut -d '-' -f1)
  fi
