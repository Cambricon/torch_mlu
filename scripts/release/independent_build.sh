...
PYTORCH_VERSION="2.3.0" # set to native release version(eg: 2.1.0, 2.2.0) or branch(eg: release/2.1, release/2.2) in catch develop branch
CATCH_BRANCH='r2.3_develop'
VISION_VERSION='0.18.0'  # set to native release version(eg: 1.6.0) in catch develop branch
AUDIO_VERSION='2.3.0'  # set to native release version(eg: 2.1.0) in catch develop branch
PYTORCH_MODELS_BRANCH="main"
BUILDER_DOCKER="yellow.hub.cambricon.com/pytorch/base/x86_64/manylinux:native-manylinux-builder-cpu-2.3"
DRIVER_VERSION=NULL
...
fetch_neuware_ver_func() {
  ...
  if [ $GENESIS_TAG != "release" ] ; then
      GENESIS_TAG=$(echo $GENESIS_VERSION | cut -d '-' -f1)
  fi
...
build_wheel_func(){
  ...
  container_name=$(docker run \
    ...
    -e CATCH_BRANCH=${CATCH_BRANCH} \
    -e CATCH_COMMIT_ID=${CATCH_COMMIT_ID} \
    --tty \
    --detach \
    --net=host \
...
pack_src_func(){
  ...
  pytorch_commit_id=`cat $CATCH_PATH/pytorch_patches/commit_id | grep pytorch | awk -F ":" '{print$2}'`
  pushd ./pytorch
  ...
  # step 3: remove docs in catch and pytorch_models/Training/tools/internal.
