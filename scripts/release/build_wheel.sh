...
function checkEnv() {
    ...
    if [[ -z "${CATCH_BRANCH}" ]]; then
        echo "ERROR : CATCH_BRANCH is not set"
        exit 1
    fi
    if [[ -z "${CATCH_COMMIT_ID}" ]]; then
        echo "ERROR : CATCH_COMMIT_ID is not set"
        exit 1
    fi
}
...
CATCH_HOME="/torch_mlu"
bash torch_mlu/.jenkins/pipeline/enable_git_url_cache.sh
...
if [[ "$PYTORCH_VERSION" == "main" ]] || [[ "$PYTORCH_VERSION" == "release"* ]]; then
    ...
    kineto_commit_id=`cat ./torch_mlu/pytorch_patches/commit_id | grep kineto | awk -F ":" '{print$2}'`
    if [[ -n "${pytorch_commit_id}" ]]; then
        pushd "$PYTORCH_HOME"
        git checkout "${pytorch_commit_id}"
        git submodule sync
        git submodule update --init --recursive
        if [[ -n "${kineto_commit_id}" ]]; then
            pushd "./third_party/kineto/"
            git checkout "${kineto_commit_id}"
            git submodule sync
            git submodule update --init --recursive
	    popd
        fi
    ...
    git clone --recursive https://github.com/pytorch/vision.git --single-branch -b main
    pushd "/vision"
    git checkout "$(cat /pytorch/.github/ci_commit_pins/vision.txt)"
    git submodule sync
    git submodule update --init --recursive
    popd
    git clone --recursive https://github.com/pytorch/audio.git --single-branch -b main
    pushd "/audio"
...
#######################################################
pip install /$WHEELHOUSE_DIR/torch*linux*.whl
