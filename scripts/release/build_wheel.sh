...
if compgen -G "${CATCH_HOME}/pytorch_patches/*diff" > /dev/null; then
    echo "There are patches to apply, clone pytorch source..."
    bash ${CATCH_HOME}/.jenkins/pipeline/enable_git_url_cache.sh
    ...
    version_type=`cat ${CATCH_HOME}/scripts/version.info | grep version_type | awk -F ":" '{print $2}'`
    pytorch=`cat ${CATCH_HOME}/scripts/version.info | grep pytorch | awk -F ":" '{print $2}'`
    vision=`cat ${CATCH_HOME}/scripts/version.info | grep vision | awk -F ":" '{print $2}'`
    audio=`cat ${CATCH_HOME}/scripts/version.info | grep audio | awk -F ":" '{print $2}'`
