...
class _DistTestBase(object):  # pylint: disable=R0205, R0904
    ...
    def _test_reduce_complex_helper(self, group, rank, op, expected_value):
        torch.mlu.set_device(rank % torch.mlu.device_count())
        ...
        if rank == src:
            real = torch.tensor(expected_value, dtype=torch.float32)
            imag = real
    ...
    def test_reduce_sum_complex(self):
        ...
        self._test_reduce_complex_helper(
            group, rank, dist.ReduceOp.SUM, n * (n + 1) / 2
        )

    # @unittest.skip("not test")
    def test_reduce_avg_complex(self):
        torch.manual_seed(1)
        group, rank = self._init_global_test()
        n = len(group)
        self._test_reduce_complex_helper(group, rank, dist.ReduceOp.AVG, (n + 1) / 2)
...
as input tensor",
    ...
    def test_batch_isend_irecv_op_coalesced_cncl(self):
        self._barrier()
        _, rank = self._init_global_test()
        torch.mlu.set_device(rank % torch.mlu.device_count())
        p2p_op_list = []
        ...
        if rank == 0 or rank == 1:
            for val in ["1", "0"]:
                ...
                for i in range(num_coalesced_ops):
                    send_tensor_list.append(torch.zeros(10, 10).mlu())
                    recv_tensor_list.append(torch.ones(10, 10).mlu())
                    if rank == 0:
                        send_op = dist.P2POp(dist.isend, send_tensor_list[i], 1)
                        p2p_op_list.append(send_op)
                    elif rank == 1:
                        recv_op = dist.P2POp(dist.irecv, recv_tensor_list[i], 0)
                        p2p_op_list.append(recv_op)
                ...
                for req in reqs:
                    req.wait()
                if rank == 1:
                    for i in range(len(recv_tensor_list)):
