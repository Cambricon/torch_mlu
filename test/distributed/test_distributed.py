...
at::Tensor reduceSum(at::Tensor& input, int64_t cnclComm) {
  ...
  void* input_ptr = torch_mlu::mlu_data_ptr(input_impl);
...
class _DistTestBase(object):  # pylint: disable=R0205, R0904
    ...
    def test_all_reduce_max_with_profiler(self):
        group, rank = self._init_global_test()
        with torch.autograd.profiler.profile(use_device="mlu", use_kineto=True) as p:
            self._test_all_reduce_helper(
                group,
                rank,
                dist.ReduceOp.MAX,
    ...
    def test_reduce_scatter_tensor_continuous(self):
        ...
        for dtype in dtype_list :
            input_tensors = list(torch.arange(self.world_size).type(dtype).mlu().chunk(self.world_size))
    ...
    def test_reduce_scatter_tensor_not_contiguous(self):
        ...
        torch.mlu.set_device(rank % torch.mlu.device_count())
        for dtype in dtype_list :
            input_tensors = [torch.tensor([i for i in range(128)]).type(dtype).mlu().as_strided([64,2], [1,64]) for i in range(self.world_size)]
            output_tensor = torch.reshape(torch.tensor([0 for i in range(128)]),(64, 2)).type(dtype).mlu()
            expected_tensor = (torch.tensor([i*self.world_size for i in range(128)])).type(dtype).as_strided([64, 2], [1, 64]) 
    ...
    def test_all_gather_tensor_continuous(self):
        ...
        torch.mlu.set_device(rank % torch.mlu.device_count())
        for dtype in dtype_list :
            output_tensors = list(torch.zeros(self.world_size).type(dtype).mlu().chunk(self.world_size))
            input_tensor = torch.tensor([rank]).type(dtype).mlu()
            expected_tensors = list(torch.arange(self.world_size).type(dtype).chunk(self.world_size))
    ...
    def test_all_gather_tensor_not_contiguous(self):
        ...
        torch.mlu.set_device(rank % torch.mlu.device_count())
        for dtype in dtype_list :
            output_tensors = [torch.zeros(128).type(dtype).mlu().as_strided([64,2], [1,64]) for i in range(self.world_size)]
            input_tensor = torch.tensor([i for i in range(128)]).type(dtype).mlu()
            expected_tensors = [torch.reshape((torch.tensor([i for i in range(128)])).type(dtype), (64, 2)) for i in range(self.world_size)]
            dist.all_gather(output_tensors, input_tensor)
            for t1, t2 in zip(output_tensors, expected_tensors):
                self.assertEqual(t1.cpu(), t2)
...
as input tensor",
    ...
    def test_distributedDataParallel_cncl_stream(self):
        ...
        self._barrier()
        print(f"Rank {rank} reached barrier before shutdown")

        pg = _get_default_group()
        pg._get_backend(torch.device(f"mlu:{rank}"))._shutdown()

        self._barrier()
        print(f"Rank {rank} reached barrier after shutdown")

        if new_group is not None:
            new_group._get_backend(torch.device(f"mlu:{rank}"))._shutdown()
        if rank == 0:
            cncl_streams_0 = torch.mlu.cncl_stream(f"mlu:{rank}")
            self.assertEqual(len(cncl_streams_0), 0)
        if rank == 1:
            cncl_streams_1 = torch.mlu.cncl_stream(f"mlu:{rank}")
            self.assertEqual(len(cncl_streams_1), 0)
        if rank == 2:
            cncl_streams_2 = torch.mlu.cncl_stream(f"mlu:{rank}")
            self.assertEqual(len(cncl_streams_2), 0)
        if rank == 3:
            cncl_streams_3 = torch.mlu.cncl_stream(f"mlu:{rank}")
            self.assertEqual(len(cncl_streams_3), 0)
        self._barrier()
    ...
    def test_abnormal_and_api(self):
        ...
        with self.assertRaisesRegex(ValueError, "Cannot use ReduceOp.BAND with CNCL"):
