...
class buildAdd(nn.Module):  # pylint: disable=W0223
    r"""
    graph：
      add --> split --> transpose --> permute --> select --> add
    """

    def __init__(self, shape):
        super(buildAdd, self).__init__()
...
class TestOneWayNetOp(TestCase):
    # @unittest.skip("not test")
    @testinfo()
    def test_one_way(self):
        # print('----one-way structure----')
        for d in range(6):
            dim = d + 1
            shape = ()
            for _ in range(1, dim + 1):
                ran_d = random.randint(3, 25)
                shape = shape + (ran_d,)
            data = torch.randn(shape, dtype=torch.float)
            in_cpu = copy.deepcopy(data)
            in_mlu = self.to_mlu(data)
            net_cpu = buildAdd(shape)
            out_cpu = net_cpu(in_cpu)
            out_mlu = net_cpu(in_mlu)
            self.assertTensorsEqual(out_cpu, out_mlu.cpu().float(), 0.03, use_MSE=True)

    # @unittest.skip("not test")
    @testinfo()
    def test_one_way_channel_last(self):
        # print('----one-way structure----')
        shape = (4, 5, 6, 7)
