...
class TestMaxOp(TestCase):
    ...
    @testinfo()
    def test_amax_dim_and_out(self):
        shape_list = [
            (2, 3, 4),
            (1, 3, 224),
            (1, 3, 1, 1, 1),
        ...
        for func in func_list:
            for i, _ in enumerate(shape_list):
                ...
                out_cpu = torch.amax(func(x), dim_list[i], keepdim=type_list[i])
                out_mlu = torch.amax(
                ...
                # amax sorting algorithm for mlu is different from cpu,
                # when value is the same the amax index may be different,
                # in this case, index test is not included for amax in unit test.
        ...
        for func in func_list:
            for shape in shape_list:
                for dtype in dtype_list:
                    ...
                    out_cpu = torch.amax(func(x))
                    out_mlu = torch.amax(func(self.to_device(x)))
                    self.assertTensorsEqual(
                        out_cpu.float(), out_mlu.float().cpu(), 0.0, use_MSE=True
                    )
        # test scalar
        x = torch.randn(())
        out_cpu = torch.amax(x)
        out_mlu = torch.amax(self.to_device(x))
        self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0.0, use_MSE=True)

    # @unittest.skip("not test")
    @testinfo()
    def test_amax_empty_tensor(self):
        input = torch.randn(2, 0, 4)
        out_cpu = torch.amax(input, dim=2)
        out_mlu = torch.amax(self.to_device(input), dim=2)
    ...
    def test_amax_bfloat16(self):
        ...
        out_cpu = torch.amax(left_cpu, 1, keepdim=True)
        out_mlu = torch.amax(left_mlu, 1, keepdim=True)
        ...
        torch.amax(left, 1, keepdim=True, out=output_cpu)
        torch.amax(left_mlu, 1, keepdim=True, out=output_mlu)
        self.assertTensorsEqual(
