...
class TestCoalesceOp(TestCase):
    ...
    def compare_tensors(self, sparse_tensor_cpu, sparse_tensor_mlu, tolerance):
        self.assertTensorsEqual(sparse_tensor_cpu._values(),
                                sparse_tensor_mlu.cpu()._values(),
                                tolerance,
                                use_MSE=True)
        self.assertTensorsEqual(sparse_tensor_cpu._indices(),
                                sparse_tensor_mlu.cpu()._indices(), 0)
    ...
    def generate_sparse_tensors(self, num_tensors, total_dim_range, min_dim,
                                max_dim, nnz_range, is_uncoalesced, device,
                                dtype):
        """
        ...
        for _ in range(num_tensors):
            ...
            for j in range(sparse_dim):
                unique_indices = torch.randint(0,
                                               size[sparse_indices[j]],
                                               (nnz, ),
                                               device=device)
            ...
            if not is_uncoalesced:
                x = x.coalesce(
                )  # Coalesce to remove duplicate indices if needed
            tensors.append(x)
    ...
    def test_Coalesce(self):
        ...
        for dtype in dtype_list:
            tensors = self.generate_sparse_tensors(num_tensors,
                                                   total_dim_range, min_dim,
                                                   max_dim, nnz_range,
                                                   is_uncoalesced, device,
                                                   dtype)
    ...
    def test_Coalesce_empty_v(self):
        ...
        for dtype in dtype_list:
            ...
            out_cpu = sparse_tensor.coalesce()
            out_mlu = sparse_tensor.to("mlu").coalesce()
            self.assertTensorsEqual(out_cpu._indices(),
                                    out_mlu.cpu()._indices(), 0)
