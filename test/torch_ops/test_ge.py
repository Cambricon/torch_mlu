...
class TestGeOp(TestCase):
    ...
    def test_ge_channel_last(self):
        ...
        for t in type_list:
            for shape1, shape2 in [((5, 3, 4, 1), (1, 3, 4, 1))]:
                ...
                out_cpu = torch.ge(x, z)
                out_mlu = torch.ge(self.to_mlu(x), self.to_mlu(z))
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu.cpu().float(), 0.0, use_MSE=True
                )
    ...
    def test_ge_not_dense(self):
        ...
        for t in type_list:
            for shape1, shape2 in [
                ...
                x = torch.randn(shape1).to(t)
                y = torch.randn(shape2).to(t)
                out_cpu = torch.ge(x[:, :, :2], y[:, :2])
                out_mlu = torch.ge(self.to_mlu(x)[:, :, :2], self.to_mlu(y)[:, :2])
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu.cpu().float(), 0.0, use_MSE=True
                )
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_ge_inplace_channel_last(self):
        type_list = [
            torch.bool,
            torch.float,
            torch.int,
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_ge_inplace_not_dense(self):
        type_list = [
            torch.bool,
            torch.float,
            torch.int,
    ...
    def test_ge_out(self):
        ...
        for t in type_list:
            for shape1, shape2 in [
                ...
                torch.ge(x, y, out=out_tmpcpu)
                torch.ge(self.to_mlu(x), self.to_mlu(y), out=out_tmpmlu)
                self.assertTensorsEqual(
    ...
    def test_ge_scalar(self):
        ...
        for t in type_list:
            for shape in [
                ...
                out_cpu = torch.ge(x, y)
                out_mlu = torch.ge(self.to_mlu(x), y)
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu.cpu().float(), 0.0, use_MSE=True
                )
                out_cpu = x >= y
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_ge_inplace_scalar(self):
        type_list = [
            torch.bool,
            torch.float,
            torch.int,
    ...
    def test_ge_out_scalar(self):
        ...
        for t in type_list:
            for shape in [
                ...
                torch.ge(x, y, out=out_tmpcpu)
                torch.ge(self.to_mlu(x), y, out=out_tmpmlu)
                self.assertTensorsEqual(
                    out_tmpcpu.float(), out_tmpmlu.cpu().float(), 0.0, use_MSE=True
                )
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_ge_and_ge_out_after_permute(self):
        for i in range(5):
            dimention_list = []
    ...
    def test_ge_mixed_type(self):
        ...
        for input1_type, input2_type, shape in product_list:
            ...
            ouput = torch.ge(a, b)
            ouput_mlu = torch.ge(a.mlu(), b.mlu())
            self.assertTensorsEqual(ouput, ouput_mlu.cpu(), 0, use_MSE=True)

    @testinfo()
    @unittest.skipUnless(TEST_BFLOAT16, "Bfloat16 only support on MLU5xx")
    def test_ge_bfloat16(self):
        ...
        out_cpu = torch.ge(left, right)
        out_mlu = torch.ge(left.mlu(), right.mlu())
        self.assertTensorsEqual(
            out_cpu.float(), out_mlu.cpu().float(), 0.0, use_MSE=True
        )
