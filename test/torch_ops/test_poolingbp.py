...
class TestPoolbpOp(TestCase):
    ...
    @largeTensorTest("29GB")
    def test_maxpool2d_backward_batch_large(self):
        ...
        output_mlu.backward(self.to_mlu_dtype(grad, dtype), retain_graph=True)
        grad_mlu = input_mlu_t.grad
        self.assertTensorsEqual(grad_cpu, grad_mlu.cpu().float(), 0.003, use_RAE=True)
    ...
    def test_maxpool3d_backward_batch_large(self):
        in_shape = (4, 1025, 32, 1024, 32)
        kernel = 4
        stride = 2
        padding = 0
        return_indices = True
    ...
    @largeTensorTest("29GB")
    def test_avgpool3d_backward_batch_large(self):
        ...
        output_mlu = avg_pool(input_mlu_t.to("mlu"))
        self.assertTensorsEqual(
            output_cpu, output_mlu.cpu().float(), 3e-3, use_MSE=True
        )
        output_mlu.backward(self.to_mlu_dtype(grad, dtype), retain_graph=True)
        grad_mlu = input_mlu_t.grad
        self.assertTensorsEqual(grad_cpu, grad_mlu.float(), 0.003, use_RAE=True)
