...
class TestTrilOp(TestCase):
    # @unittest.skip("not test")
    @testinfo()
    def test_torch_tril(self):
        shape_list = [
            (3, 3),
            (5, 8),
            (8, 10),
        ...
        for shape in shape_list:
            ...
            for diagonal in range(-1 * min_, min_):
                ...
                out_cpu = torch.tril(x, diagonal)
                out_mlu = torch.tril(self.to_mlu(x), diagonal)
                self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0.0, use_MSE=True)

    # @unittest.skip("not test")
    @testinfo()
    def test_torch_tril_dtype(self):
        dtype_list = [
            torch.float,
            torch.half,
            torch.int,
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_torch_tril_not_dense(self):
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_tensor_tril_inplace(self):
    ...
    def test_tril_out(self):
        ...
        for shape in shape_list:
            ...
            for diagonal in range(-1 * min_, min_):
                ...
                out_cpu = torch.tril(x, diagonal)
                torch.tril(self.to_mlu(x), diagonal, out=out_mlu)
                self.assertEqual(out_mlu_data_ptr, out_mlu.data_ptr())
                self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0.0, use_MSE=True)

    # @unittest.skip("not test")
    @testinfo()
    def test_tril_exception(self):
