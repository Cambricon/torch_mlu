...
class TestExp2Op(TestCase):
    # @unittest.skip("not test")
    @testinfo()
    def test_exp2_contiguous(self):
        shape_list = [(16, 384, 3072), (16, 0, 88)]
        data_types = [
            torch.float,
            torch.half,
            torch.double,
            # TODO: MLU pow don't support tensor dtype Complex
            # torch.cfloat, torch.cdouble
        ]
        for shape in shape_list:
            for data_type in data_types:
                x = torch.randn(shape, dtype=torch.float)
                out_cpu = torch.exp2(x)
                out_mlu = torch.exp2(self.to_mlu_dtype(x, data_type))
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_exp2_channel_last(self):
        shape_list = [(2, 3, 3, 4)]
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_exp2_not_dense(self):
        shape_list = [(2, 3, 3, 4)]
        data_types = [torch.float, torch.half, torch.double]
        for shape in shape_list:
            for data_type in data_types:
                x = torch.randn(shape, dtype=torch.float)
                out_cpu = torch.exp2(x[:, :, :, :2])
                out_mlu = torch.exp2(self.to_mlu_dtype(x, data_type)[:, :, :, :2])
                self.assertTensorsEqual(
                    out_cpu, out_mlu.cpu().float(), 0.003, use_MSE=True
                )
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_exp2_inplace_contiguous(self):
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_exp2_inplace_channel_last(self):
        shape_list = [(2, 3, 3, 4)]
        data_types = [torch.float, torch.half]
        for shape in shape_list:
            for data_type in data_types:
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_exp2_inplace_not_dense(self):
        shape_list = [(2, 3, 3, 4)]
        data_types = [torch.float, torch.half]
        for shape in shape_list:
            for data_type in data_types:
        ...
        for out_shape in out_shapes:
            for shape in shape_list:
                for data_type in data_types:
                    ...
                    torch.exp2(x, out=out_cpu)
                    torch.exp2(x_mlu, out=out_mlu)
                    self.assertTensorsEqual(
                        out_cpu, out_mlu.cpu().float(), 0.003, use_MSE=True
                    )
    ...
    def test_exp2_type(self):
        ...
        for shape in shape_list:
            for type in type_list:
                ...
                if type == torch.half:
                    x_cpu = x_cpu.float()
                out_cpu = torch.exp2(x_cpu)
                out_mlu = torch.exp2(x_mlu)
                self.assertTensorsEqual(
