...
class TestOps(TestCase):
    ...
    def test_sum_dtype(self):
        ...
        for t in type_list:
            for out_dtype in out_dtype_list:
                ...
                out_mlu = x.to("mlu").sum(dim=1, keepdim=True, dtype=out_dtype)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_sum_opt_dtype(self):
        shape = (2, 3, 4)
        type_list = [torch.int, torch.short, torch.int8, torch.long, torch.uint8]
        opt_list = [torch.int, torch.float, torch.long]
        for t, opttype in product(type_list, opt_list):
            x = (torch.randn(shape, dtype=torch.float) * 10000).to(t)
            out_cpu = x.sum(dim=1, keepdim=True, dtype=opttype)
            out_mlu = x.to("mlu").sum(dim=1, keepdim=True, dtype=opttype)
            self.assertEqual(out_cpu.dtype, out_mlu.dtype)
            self.assertTensorsEqual(
                out_cpu.float(), out_mlu.cpu().float(), 0.003, use_MSE=True
            )
        ...
        for t, opttype in product(type_list, opt_list):
            x = (torch.randn(shape, dtype=torch.float) * 10000).to(t)
            out_cpu = x.sum(dtype=opttype)
            out_mlu = x.to("mlu").sum(dtype=opttype)
    ...
    def test_sum_backward(self):
        ...
        for shape in shape_list:
            ...
            for item in product(keepdim_list, range(-dim_len, dim_len)):
                ...
                out_cpu = torch.sum(x, item[1], keepdim=item[0])
                ...
                out_mlu = torch.sum(x_mlu, item[1], keepdim=item[0])
    ...
    def test_sum_bfloat16(self):
        ...
        out_cpu = torch.sum(left_cpu)
        out_mlu = torch.sum(left_mlu)
        grad = torch.randn(out_cpu.shape)
        grad_mlu = copy.deepcopy(grad).to("mlu")
        out_cpu.backward(grad)
        out_mlu.backward(grad_mlu)
        ...
        left_cpu.grad.zero_()
        left_mlu.grad.zero_()
        out_cpu = torch.sum(left_cpu, 1, keepdim=True)
        out_mlu = torch.sum(left_mlu, 1, keepdim=True)
        grad = torch.randn(out_cpu.shape)
        grad_mlu = copy.deepcopy(grad).to("mlu")
        out_cpu.backward(grad)
        out_mlu.backward(grad_mlu)
