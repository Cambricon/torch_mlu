...
class TestCummaxOp(TestCase):
    ...
    def test_cummax(self):
        ...
        for memory_format, shape, dim in product(*loop_var):  # pylint: disable=C0200
            ...
            # test cummax(tensor, tensor)
            out_cpu = torch.cummax(x_cpu, dim)
            out_mlu = torch.cummax(x_mlu, dim)
            torch.cummax(x_cpu, dim, out=(y_value_cpu, y_indice_cpu))
            torch.cummax(x_mlu, dim, out=(y_value_mlu, y_indice_mlu))
    ...
    def test_cummax_backward(self):
        ...
        for memory_format, shape, dim in product(*loop_var):  # pylint: disable=C0200
            ...
            input_mlu = x_mlu.mlu()
            # test cummax(tensor, tensor)
            out_cpu = x_cpu.cummax(dim)
            out_mlu = input_mlu.cummax(dim)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_cummax_not_dense(self):
        shape_list = [
            (12, 10, 2, 2, 4),
            (10, 3, 32, 32),
            (2, 3, 4),
        ...
        for shape, dtype, dim in product(*loop_var):
            ...
            if len(shape) == 4:
                ...
                x = x[:, :, : int(shape[-1] / 2)]
                x_mlu = x_mlu[:, :, : int(shape[-1] / 2)]
            out_cpu = x.cummax(dim=dim)
            out_mlu = x_mlu.cummax(dim=dim)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_cummax_support_dtype(self):
        support_dtype = [
            torch.uint8,
            torch.int8,
            torch.int16,
        ...
        for dtype in support_dtype:
            ...
            if str(dtype) != "torch.float16":
                out_values_c, out_indices_c = torch.cummax(input_c, 1)
            out_values_m, out_indices_m = torch.cummax(input_m, 1)
            if str(dtype) != "torch.float16":
                self.assertTensorsEqual(
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_cummax_not_support_dtype(self):
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_cummax_zero(self):
        s = torch.randn(size=[], dtype=torch.float)
        out_cpu = torch.cummax(s, 0)
        out_mlu = torch.cummax(s.to("mlu"), 0)
        self.assertTensorsEqual(
            out_cpu[0], out_mlu[0].cpu(), 0, use_MSE=True
        )  # float type precision : 0
        self.assertTensorsEqual(
    ...
    def test_cummax_nan(self):
        ...
        for x in input_list:
            out_cpu = torch.cummax(x, 0)
            out_mlu = torch.cummax(x.to("mlu"), 0)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_cummax_exception(self):
        ...
        with self.assertRaisesRegex(
            ...
            torch.cummax(t_mlu, 0, out=(values, indices))
    ...
    def test_cummax_large(self):
        shape = (4, 1025, 1024, 1024)
        dim = 0
        x_cpu = torch.randn(shape, dtype=torch.half)
        x_mlu = self.to_mlu(x_cpu)
        out_cpu = torch.cummax(x_cpu.float(), dim)
        out_mlu = torch.cummax(x_mlu, dim)
