...
class TestArgminOp(TestCase):
    def check_result(self, x, ind_cpu, ind_mlu, dim):
        # min sorting algorithm for mlu is different from cpu,
        # when the min result has multi-

        if dim is None:
            # None dim means index is one number for min-value in full storage.
            x_tr = x.view(-1)
            ...
            # and then the arange t will select min-value due to the index,
            # so we can check if mlu and cpu choose the same min-value.
    ...
    def test_argmin(self):
        ...
        for dtype, shape, dim, keepdim, mode in product(*list_list):
            ...
            if dim is not None and dim >= len(shape):
                dim = dim % len(shape)
            out_cpu = torch.argmin(mode(x), dim, keepdim=keepdim)
            out_mlu = torch.argmin(mode(self.to_device(x)), dim, keepdim=keepdim)
            out_mlu = out_mlu.cpu()
    ...
    def test_argmin_out(self):
        ...
        for in_dtype, out_dtype, dim, keepdim, input_shape in product(*params_list):
            ...
            output_cpu = torch.argmin(input=x, dim=dim, keepdim=keepdim, out=y)
            output_mlu = torch.argmin(
    ...
    def test_argmin_bfloat16(self):
        left_cpu = torch.testing.make_tensor(
            (2, 10, 24), dtype=torch.bfloat16, device="cpu"
        )
        left_mlu = left_cpu.mlu()
        out_cpu = torch.argmin(left_cpu, 1, keepdim=True)
        out_mlu = torch.argmin(left_mlu, 1, keepdim=True)
        self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0.0, use_MSE=True)
        output_cpu = torch.testing.make_tensor(
            out_cpu.shape, dtype=torch.long, device="cpu"
        )
        output_mlu = output_cpu.mlu()
        torch.argmin(left_cpu, 1, keepdim=True, out=output_cpu)
        torch.argmin(left_mlu, 1, keepdim=True, out=output_mlu)
    ...
    def test_argmin_large(self):
        ...
        for dtype, shape, dim, keepdim in product(*list_list):
            ...
            if dim is not None and dim >= len(shape):
                dim = dim % len(shape)
            out_cpu = torch.argmin(x, dim, keepdim=keepdim)
            out_mlu = torch.argmin(self.to_device(x), dim, keepdim=keepdim)
            out_mlu = out_mlu.cpu()
            self.assertEqual(out_cpu.dtype, out_mlu.dtype)
            self.assertEqual(out_cpu.size(), out_mlu.size())
