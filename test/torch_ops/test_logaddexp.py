...
class TestLogAddExpOp(TestCase):
        ...
        for type in type_list:
            for input_shape in shape_list:
                for other_shape in shape_list:
                    ...
                    out_cpu = torch.logaddexp(input, other)
                    out_mlu = torch.logaddexp(input_mlu, other_mlu)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_logaddexp_channels_last(self):
        shape_list = [(2, 3, 4, 5)]
    ...
    @skipDtypeNotSupport("bfloat16", "half")
    @testinfo()
    def test_logaddexp_unsupported(self, type):
        shape = (1, 3)
        if type == "bfloat16":
            t = torch.bfloat16
        else:
