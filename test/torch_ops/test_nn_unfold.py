...
class TestNNUnfoldOp(TestCase):
    ...
    def test_nn_unfold(self):
        ...
        for N, Ci, H, W, K, P, S, D, dtype, func in product_list:
            input_cpu, input_mlu = self.TensorGenerator((N, Ci, H, W), dtype, func)
            input_cpu = torch.nn.Parameter(input_cpu)
            input_mlu = torch.nn.Parameter(input_mlu)
            unfold = torch.nn.Unfold(
                kernel_size=(K, K), stride=(S, S), padding=(P, P), dilation=(D, D)
            )
            output_cpu = unfold(input_cpu)
            output_mlu = unfold(input_mlu)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_nn_unfold_invalid_arg(self):
        # input wrong dimension

        unfold = torch.nn.Unfold(kernel_size=(2, 3))
        with self.assertRaisesRegex(
            RuntimeError, r"but its components must be at least one"
        ):
            unfold(torch.randn(1, 5, 2).mlu())
        ...
        with self.assertRaisesRegex(
            ...
            unfold = torch.nn.Unfold(kernel_size=(2, 3))
            unfold(torch.randn(1, 2, 2, 2).mlu())

        with self.assertRaisesRegex(
            RuntimeError, r"but its components must be at least one"
        ):
            unfold = torch.nn.Unfold(kernel_size=(5, 3), padding=(1, 1))
            unfold(torch.randn(1, 2, 2, 3).mlu())

        with self.assertRaisesRegex(
            RuntimeError, r"but its components must be at least one"
        ):
            unfold = torch.nn.Unfold(
                kernel_size=(1, 3), padding=(1, 1), dilation=(1, 2)
            )
            unfold(torch.randn(1, 2, 2, 2).mlu())
    ...
    @testinfo()
    def test_nn_unfold_bfloat16(self):
        # only support 3 dims or 4 dims input.
        N_lst = [0, 3, 5]
        Ci_lst = [1, 3]
        H_lst = [20, 32]
