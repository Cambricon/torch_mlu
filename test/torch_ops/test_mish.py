...
class TestMishOp(TestCase):
    # @unittest.skip("not test")
    @testinfo()
    def test_mish_contiguous(self):
        ...
        for in_shape in shape_list:
            for typeId in type_list:
                ...
                x = data.to(typeId)
                mish_layer = nn.Mish()
                output_cpu = mish_layer(data)
                output_mlu = mish_layer(self.to_mlu(x))
                self.assertTensorsEqual(
                    output_cpu, output_mlu.cpu(), 0.003, use_MSE=True
                )
                # test function:
                output_cpu_f = F.mish(data, inplace=False)
                output_mlu_f = F.mish(self.to_mlu(x), inplace=False)
                self.assertTensorsEqual(
    ...
    def test_mish_permute(self):
        ...
        for in_shape in [
            ...
            output_cpu = F.mish(input_, inplace=False)
            output_mlu = F.mish(input_mlu, inplace=False)
            F.mish(input_inplace_, inplace=True)  # test inplace operation
            F.mish(input_mlu_inplace_, inplace=True)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_mish_channel_last(self):
        ...
        for in_shape in shape_list:
            for typeId in type_list:
                ...
                mish_layer = nn.Mish()
                output_cpu = mish_layer(data)
                output_mlu = mish_layer(x_mlu)
                self.assertTensorsEqual(
                    output_cpu, output_mlu.cpu(), 0.003, use_MSE=True
                )
                # test function:
                output_cpu_f = F.mish(data, inplace=False)
                output_mlu_f = F.mish(x_mlu, inplace=False)
                self.assertTensorsEqual(
                    output_cpu_f, output_mlu_f.cpu(), 0.003, use_MSE=True
                )
    ...
    def test_mish_not_dense(self):
        ...
        for in_shape in shape_list_not_dense:
            for typeId in type_list:
                ...
                mish_layer = nn.Mish()
                output_cpu = mish_layer(x_cpu)
                output_mlu = mish_layer(x_mlu)
                self.assertTensorsEqual(
                    output_cpu, output_mlu.cpu(), 0.003, use_MSE=True
                )
                # test function:
                output_cpu_f = F.mish(x_cpu, inplace=False)
                output_mlu_f = F.mish(x_mlu, inplace=False)
                self.assertTensorsEqual(
                    output_cpu_f, output_mlu_f.cpu(), 0.003, use_MSE=True
                )
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_mish_inplace_contiguous(self):
        shape_list = [(168), (45, 50), (15, 224, 224), (1, 3, 224, 224)]
        for in_shape in shape_list:
            for typeId in type_list:
                data = torch.randn(in_shape, dtype=torch.float)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_mish_inplace_not_dense(self):
        shape_list_not_dense = [(45, 100), (15, 224, 448), (1, 3, 224, 448)]
        for in_shape in shape_list_not_dense:
            for typeId in type_list:
                data = torch.randn(in_shape, dtype=torch.float)
    ...
    def test_mish_backward(self):
        ...
        for shape in shape_list:
            ...
            x_mlu = data.mlu()
            mish_layer = nn.Mish()
            out_cpu = mish_layer(data)
            out_mlu = mish_layer(x_mlu)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_mish_backwark_permute(self):
        input_cpu = torch.randn((4, 3, 2, 1), dtype=torch.float, requires_grad=True)
        out_cpu = F.mish(input_cpu)
        out_mlu = F.mish(input_cpu.to("mlu"))
