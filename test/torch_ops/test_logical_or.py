...
class TestLogicalOrOp(TestCase):
    ...
    def test_logical_or(self):
        ...
        for t in type_list:
            for shape1, shape2 in [
                ...
                out_cpu = torch.logical_or(x, y)
                out_mlu = torch.logical_or(self.to_mlu(x), self.to_mlu(y))
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_logical_or_not_dense(self):
        type_list = [
            torch.bool,
            torch.float,
            torch.int,
        ...
        for t in type_list:
            for shape1, shape2 in [
                ...
                x = torch.randn(shape1).to(t)
                y = torch.randn(shape2).to(t)
                out_cpu = torch.logical_or(x, y[:, :, :, 10:36])
                y_mlu = self.to_mlu(y)
                out_mlu = torch.logical_or(self.to_mlu(x), y_mlu[:, :, :, 10:36])
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu.cpu().float(), 0.0, use_MSE=True
                )
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_logical_or_channels_last(self):
        type_list = [
            torch.bool,
            torch.float,
            torch.int,
    ...
    def test_logical_or_out(self):
        ...
        for t in type_list:
            for shape1, shape2 in [
                ...
                torch.logical_or(x, y, out=out_cpu)
                torch.logical_or(self.to_mlu(x), self.to_mlu(y), out=out_mlu)
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu.cpu().float(), 0.0, use_MSE=True
                )
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_logical_or_mixed_type(self):
        ...
        for input1_type, input2_type, shape in product_list:
            ...
            ouput = torch.logical_or(a, b)
            ouput_mlu = torch.logical_or(a.mlu(), b.mlu())
            self.assertTensorsEqual(ouput, ouput_mlu.cpu(), 0, use_MSE=True)

    @testinfo()
    @unittest.skipUnless(TEST_BFLOAT16, "Bfloat16 only support on MLU5xx")
    def test_logical_or_bfloat16(self):
        ...
        out_cpu = torch.logical_or(left, right)
        out_mlu = torch.logical_or(left.mlu(), right.mlu())
        self.assertTensorsEqual(
            out_cpu.float(), out_mlu.cpu().float(), 0.0, use_MSE=True
        )
    ...
    def test_logical_or_large(self):
        left = torch.testing.make_tensor(
