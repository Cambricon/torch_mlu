...
class TestLstmCellOp(TestCase):
    # @unittest.skip("not test")
    @testinfo()
    def test_lstmcell_training(self):
        input_size_v = [832, 576]
        hidden_size_v = [512, 256]
        batch_v = [128, 70]
        bias_v = [True, False]
        ...
        for input_size, hidden_size, batch, dtype, bias in product(
            ...
            c0_mlu = c0.to("mlu").to(dtype)
            hn_mlu, cn_mlu = rnn_mlu(input_mlu, (h0_mlu, c0_mlu))
            ...
            c0_mlu_grad = copy.deepcopy(c0.grad)
            w_grad_mlu = []
            for w in rnn.parameters():
            ...
            self.assertTensorsEqual(c0_grad, c0_mlu_grad.cpu(), 3e-3, use_MSE=True)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_lstmcell_training_not_contiguous(self):
        ...
        for input_size, hidden_size, batch, dtype, bias in product(
            ...
            c0 = nn.Parameter(torch.randn(batch, hidden_size * 2))
            hn, cn = rnn(input[:, ::2], (h0[:, ::2], c0[:, ::2]))
    ...
    def test_lstmcell_training_large(self):
        ...
        self.assertTensorsEqual(cn, cn_mlu.cpu().float(), 3e-3, use_MSE=True)
        ...
        self.assertTensorsEqual(c0_grad, c0_mlu_grad.cpu().float(), 3e-3, use_MSE=True)
