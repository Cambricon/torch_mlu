...
def to_fp8_saturated(x: torch.Tensor, x_scale: torch.tensor, fp8_dtype: torch.dtype):
    """
...
class TestMatmulOp(TestCase):
    ...
    @testinfo()
    @unittest.skipUnless(TEST_FLOAT8, "float8 only support on MLU6xx")
    def test_float8_basics(self, device="mlu") -> None:
    ...
    @testinfo()
    @unittest.skipUnless(TEST_FLOAT8, "float8 only support on MLU6xx")
    def test_float8_scale(self, device="mlu") -> None:
        size = (16, 16)
        ...
        self.assertEqual(out_fp8.float(), out_fp8_s.float())

    @testinfo()
    @unittest.skipUnless(TEST_FLOAT8, "float8 only support on MLU6xx")
    def test_scaled_mm_vs_emulated(self, device="mlu"):
        torch.manual_seed(42)
        input_dtype = e4m3_type
        base_dtypes = [torch.float16, torch.bfloat16, torch.float32]
        compare_type = torch.float32

        for base_dtype in base_dtypes:
            output_dtype = base_dtype

            x = torch.randn(16, 16, device="mlu", dtype=base_dtype)
            y = torch.randn(32, 16, device="mlu", dtype=base_dtype).t()
            ...
            out_emulated, output_amax_emulated = mm_float8_emulated(
                x_fp8, x_scale, y_fp8, y_scale, output_dtype
            )
            ...
            torch.testing.assert_close(
                out_scaled_mm, out_emulated, atol=atol, rtol=rtol
            )

    @testinfo()
    @unittest.skipUnless(TEST_FLOAT8, "float8 only support on MLU6xx")
    def test_float8_bias(self, device="mlu") -> None:
        (k, l, m) = (16, 48, 32)
        x = torch.rand((k, l), device=device).to(e4m3_type)
        y = torch.full((m, l), 0.25, device=device, dtype=e4m3_type).t()
        bias = torch.full((1, m), 4.0, device=device, dtype=torch.float)
        out_fp8, amax_fp8 = torch._scaled_mm(x, y)
