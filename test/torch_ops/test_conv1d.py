...
class TestConvOps(TestCase):
    ...
    def test_conv1d_exceptions(self):
        ...
        with self.assertRaises(RuntimeError) as info:
            _ = cm(x.to("mlu"))
        msg = "Input type (unsigned char) and bias type (float) should be the same"
        self.assertEqual(info.exception.args[0], msg)
    ...
    @testinfo()
    @unittest.skip("not test, see CNNLCORE-18739")
    @unittest.skipUnless(
        TEST_LARGETENSOR, "run largeTensorCases by `TEST_LARGETENSOR=TRUE` or `--large`"
    )
    @largeTensorTest("29GB")
    def test_conv1d_large(self):
        # TODO: Floating point exception (core dumped)
        bias_t = False
        N = 4 * 1025
        Ci = 32 * 1024
        HW = 32
        ...
        groups = 1
        dtype = torch.half
        x = torch.randn(N, Ci, HW, dtype=dtype)
        er = 0.003
        cm = nn.Conv1d(
            Ci,
            Co,
