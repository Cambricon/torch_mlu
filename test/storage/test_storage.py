...
class Storage(TestCase):
    ...
    def test_serialization_array_with_storage(self):
        ...
        self.assertTrue(isinstance(q_copy[0], torch.mlu.FloatTensor))
        self.assertTrue(isinstance(q_copy[1], torch.mlu.IntTensor))
        self.assertTrue(isinstance(q_copy[2], torch.mlu.FloatTensor))
        self.assertTrue(isinstance(q_copy[3], torch.storage.TypedStorage))
        # now use torch.mlu.UntypedStorage to support mlu Untyped.
        self.assertTrue(isinstance(q_copy[3].untyped(), torch.mlu.UntypedStorage))
        q_copy[1].fill_(10)
        self.assertEqual(q_copy[3], torch.mlu.IntStorage(10).fill_(10))
    ...
    def test_type(self):
        ...
        self.assertIsInstance(x.untyped().mlu(), torch.mlu.UntypedStorage)
        self.assertIsInstance(x.mlu().untyped(), torch.mlu.UntypedStorage)
        ...
        self.assertIsInstance(x.mlu(), torch.mlu.UntypedStorage)
        self.assertIsInstance(x.mlu().double(), torch.mlu.DoubleStorage)
        self.assertIsInstance(x.double().mlu(), torch.mlu.DoubleStorage)
        self.assertIsInstance(x.mlu().float(), torch.mlu.FloatStorage)
        self.assertIsInstance(x.mlu().char(), torch.mlu.CharStorage)
    ...
    def test_tensor_set_cdata(self):
        a = torch.mlu.UntypedStorage(sequence=[4, 5, 6], device="mlu")
        b = torch.mlu.UntypedStorage(sequence=[1, 2, 3], device="mlu")
    ...
    def test_storage_casts(self):
        storage = torch.mlu.IntStorage([-1, 0, 1, 2, 3, 4])
        self.assertEqual(storage.size(), 6)
        self.assertEqual(storage.tolist(), [-1, 0, 1, 2, 3, 4])
        self.assertEqual(storage.type(), "torch_mlu.mlu.IntStorage")
        ...
        self.assertEqual(floatStorage.type(), "torch_mlu.mlu.FloatStorage")
        ...
        self.assertEqual(halfStorage.type(), "torch_mlu.mlu.HalfStorage")
        ...
        self.assertEqual(longStorage.type(), "torch_mlu.mlu.LongStorage")
        ...
        self.assertEqual(shortStorage.type(), "torch_mlu.mlu.ShortStorage")
        ...
        self.assertEqual(doubleStorage.type(), "torch_mlu.mlu.DoubleStorage")
        ...
        self.assertEqual(charStorage.type(), "torch_mlu.mlu.CharStorage")
        ...
        self.assertEqual(byteStorage.type(), "torch_mlu.mlu.ByteStorage")
        ...
        self.assertEqual(boolStorage.type(), "torch_mlu.mlu.BoolStorage")
        self.assertEqual(boolStorage.int().tolist(), [1, 0, 1, 1, 1, 1])
        self.assertIs(boolStorage.dtype, torch.bool)

        complexfloat_storage = torch.mlu.ComplexFloatStorage(
        ...
        self.assertEqual(
            complexfloat_storage.type(), "torch_mlu.mlu.ComplexFloatStorage"
        )
        ...
        self.assertEqual(
            complexdouble_storage.type(), "torch_mlu.mlu.ComplexDoubleStorage"
        )
    ...
    def test_storage_pin_memory_and_is_pinned(self):
        ...
        x_UntypedStorage = torch.mlu.UntypedStorage([1, 2], device="cpu")
    ...
    def test_storage_resize(self):
        ...
        x = torch.mlu.UntypedStorage([1, 2, 3, 4], device="cpu")
        x.resize_(0)
        self.assertEqual(x.size(), 0, atol=1.0e-12, rtol=1.0e-12)
    ...
    def test_storage_fill(self):
        ...
        x = torch.mlu.UntypedStorage([1, 2, 3, 4, 5], device="mlu")
        y = x.fill_(0)
    ...
    def test_storage_weak_ref(self):
        ...
        torch.mlu.UntypedStorage._free_weak_ref(cdata)
    ...
    def test_storage_new_with_weak_ptr(self):
        for device in ["cpu", "mlu"]:
            ...
            a = torch.mlu.UntypedStorage(tmp, device=device)
            r = a._weak_ref()
            self.assertEqual(r, a._cdata)
            x = torch.mlu.UntypedStorage._new_with_weak_ptr(r)
    ...
    def test_storage_expired(self):
        ...
        cdata = x_storage._weak_ref()
        # static methods
        expired = torch.mlu.UntypedStorage._expired(cdata)
    ...
    def test_storage_incdecref(self):
        ...
        y = torch.mlu.UntypedStorage([1, 2, 3, 4], device="cpu")
        y._shared_incref()
        y._shared_decref()
    ...
    def test_storage_new_share(self):
        ...
        for strategy in sharing_strategy:
            ...
            x = torch.mlu.UntypedStorage._new_shared(size, device="cpu")
            shared = x.is_shared()
    ...
    def test_storage_new_shared_fd_cpu(self):
        a = torch.mlu.UntypedStorage(
            [189, 0, 89, 63, 227, 64, 194, 61, 7, 234, 129, 63, 29, 147, 120, 63],
            device="cpu",
        )
        fd, size = a._share_fd_cpu_()
        x = torch.mlu.UntypedStorage._new_shared_fd_cpu(fd, size)
        self.assertEqual(x.tolist(), a.tolist())
    ...
    def test_storage_new_shared_filename_cpu(self):
        ...
        a = torch.mlu.UntypedStorage(a_list, device="cpu")
        manager, handle, size = a._share_filename_cpu_()
        b = torch.mlu.UntypedStorage._new_shared_filename_cpu(manager, handle, size)
    ...
    # skip it, set_default_tensor_type is deprecated from Pytorch2.1
    @unittest.skip("not test")
    @testinfo()
    def test_constructor_dtypes(self):
        if torch.mlu.is_available():
            ...
            self.assertIs(torch.mlu.FloatStorage, torch.Storage)

            # set_default_dtype() depends on the current default tensor type
            # if default tensor type == torch.mlu.FloatTensor, torch.Storage would be torch.mlu.DoubleStorage after torch.set_default_dtype(torch.float64),
            ...
            self.assertIs(torch.mlu.DoubleStorage, torch.Storage)
            torch.set_default_dtype(default_dtype)
            torch.set_default_tensor_type(default_type)
    ...
    def test_element_size(self):
        byte = torch.mlu.ByteStorage().element_size()
        char = torch.mlu.CharStorage().element_size()
        short = torch.mlu.ShortStorage().element_size()
        int = torch.mlu.IntStorage().element_size()
        long = torch.mlu.LongStorage().element_size()
        float = torch.mlu.FloatStorage().element_size()
        double = torch.mlu.DoubleStorage().element_size()
        bool = torch.mlu.BoolStorage().element_size()
        # bfloat16 = torch.mlu.BFloat16Storage().element_size()
        complexfloat = torch.mlu.ComplexFloatStorage().element_size()
        complexdouble = torch.mlu.ComplexDoubleStorage().element_size()

        self.assertEqual(byte, torch.mlu.ByteTensor().element_size())
        self.assertEqual(char, torch.mlu.CharTensor().element_size())
        self.assertEqual(short, torch.mlu.ShortTensor().element_size())
        self.assertEqual(int, torch.mlu.IntTensor().element_size())
        self.assertEqual(long, torch.mlu.LongTensor().element_size())
        self.assertEqual(float, torch.mlu.FloatTensor().element_size())
        self.assertEqual(double, torch.mlu.DoubleTensor().element_size())
        self.assertEqual(bool, torch.mlu.BoolTensor().element_size())
    ...
    def test_from_buffer(self):
        for order in ["big", "little", "native"]:
            a = bytearray([1, 2, 3, 4])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(
            ...
            # untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.complex128)
    ...
    def test_from_file(self):
        ...
        for size in sizes:
            with tempfile.NamedTemporaryFile() as f:
                s1 = torch.mlu.UntypedStorage.from_file(f.name, True, size)
                t1 = torch.DoubleTensor(s1)
                t1 = t1.copy_(torch.randn(t1.size()))

                # check mapping
                s2 = torch.mlu.UntypedStorage.from_file(f.name, True, size)
    ...
    @testinfo()
    def test_pynew(self):
        floatStorage = torch.mlu.FloatStorage()
        self.assertEqual(floatStorage.size(), 0)
        self.assertEqual(floatStorage.type(), "torch_mlu.mlu.FloatStorage")
        self.assertIs(floatStorage.dtype, torch.float32)
        self.assertIs(
            floatStorage.element_size(), torch.mlu.FloatTensor().element_size()
        )

        floatStorage = torch.mlu.FloatStorage(8)
        ...
        floatStorage = torch.mlu.FloatStorage([1, 2, 3, 4])
        ...
        complexfloatStorage = torch.mlu.ComplexFloatStorage([1 + 1j, 2 - 1j, 3, 4])
        self.assertEqual(complexfloatStorage.size(), 4)
        self.assertEqual(
            complexfloatStorage.type(), "torch_mlu.mlu.ComplexFloatStorage"
        )
        ...
        complexdoubleStorage = torch.mlu.ComplexDoubleStorage([1 + 1j, 2 - 1j, 3, 4])
        self.assertEqual(complexdoubleStorage.size(), 4)
        self.assertEqual(
            complexdoubleStorage.type(), "torch_mlu.mlu.ComplexDoubleStorage"
        )
        ...
        mlu_untyped_storage = torch.mlu.UntypedStorage([1, 2, 3, 4, 5], device="mlu")
        mlu_new_storage = mlu_untyped_storage.new()
        self.assertTrue(mlu_new_storage.device.type == "mlu")

        cpu_untyped_storage = torch.mlu.UntypedStorage([1, 2, 3, 4, 5], device="mlu")
    ...
    def test_copy_(self):
        ...
        b.storage().copy_(a.char().storage())
        self.assertIsInstance(a.char().storage(), torch.mlu.CharStorage)
        self.assertIsInstance(b.char().storage(), torch.mlu.CharStorage)
        self.assertEqual(a.char(), b.char(), atol=1.0e-12, rtol=1.0e-12)

        b.storage().copy_(a.byte().storage())
        self.assertIsInstance(a.byte().storage(), torch.mlu.ByteStorage)
        self.assertIsInstance(b.byte().storage(), torch.mlu.ByteStorage)
        self.assertEqual(a.byte(), b.byte(), atol=1.0e-12, rtol=1.0e-12)

        b.storage().copy_(a.double().storage())
        self.assertIsInstance(a.double().storage(), torch.mlu.DoubleStorage)
        self.assertIsInstance(b.double().storage(), torch.mlu.DoubleStorage)
        self.assertEqual(a.double(), b.double(), atol=1.0e-12, rtol=1.0e-12)

        b.storage().copy_(a.long().storage())
        self.assertIsInstance(a.long().storage(), torch.mlu.LongStorage)
        self.assertIsInstance(b.long().storage(), torch.mlu.LongStorage)
        self.assertEqual(a.long(), b.long(), atol=1.0e-12, rtol=1.0e-12)

        b.storage().copy_(a.short().storage())
        self.assertIsInstance(a.short().storage(), torch.mlu.ShortStorage)
        self.assertIsInstance(b.short().storage(), torch.mlu.ShortStorage)
        self.assertEqual(a.short(), b.short(), atol=1.0e-12, rtol=1.0e-12)

        b.storage().copy_(a.half().storage())
        self.assertIsInstance(a.half().storage(), torch.mlu.HalfStorage)
        self.assertIsInstance(b.half().storage(), torch.mlu.HalfStorage)
        self.assertEqual(a.half(), b.half(), atol=1.0e-12, rtol=1.0e-12)

        b.storage().copy_(a.bool().storage())
        self.assertIsInstance(a.bool().storage(), torch.mlu.BoolStorage)
        self.assertIsInstance(b.bool().storage(), torch.mlu.BoolStorage)
    ...
    def test__write_and_new_with_file(self):
        ...
        with open("_write_file.pt", "rb") as reader:
            storage_b = torch.mlu.UntypedStorage._new_with_file(
                reader, torch._utils._element_size(tensor_a.dtype)
            )
        tensor_b = torch.mlu.FloatTensor(storage_b)
        self.assertEqual(tensor_a, tensor_b.reshape(4, 5), atol=1.0e-12, rtol=1.0e-12)

        untyped_a = torch.mlu.UntypedStorage([1, 2, 3, 4], device="cpu")
        ...
        with open("_write_file_1.pt", "rb") as reader:
            untyped_b = torch.mlu.UntypedStorage._new_with_file(
        ...
        tensor_b = torch.mlu.DoubleTensor(storage_b)
        self.assertEqual(storage_b.type(), "torch_mlu.storage.UntypedStorage")
        self.assertEqual(
        ...
        # because when the default tensor type is torch.mlu.FloatTensor, the tensor constructed by tensor_b = torch.tensor(storage_b) is a float tensor,
        ...
        typedstorage_b = torch.mlu.ComplexFloatStorage(wrap_storage=storage_b)
        tensor_b.set_(typedstorage_b)
        self.assertEqual(
        ...
        typedstorage_b = torch.mlu.ComplexDoubleStorage(wrap_storage=storage_b)
        self.assertEqual(
            tensor_a.storage().cpu(), typedstorage_b.cpu(), atol=1.0e-12, rtol=1.0e-12
        )
    ...
    def test_new_with_file1(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.seek(0)
            x_untypedstorage = torch.mlu.UntypedStorage._new_with_file(
                f, torch._utils._element_size(x.dtype)
            )
            y_untypedstorage = torch.mlu.UntypedStorage._new_with_file(
                f, torch._utils._element_size(y.dtype)
            )
            z_untypedstorage = torch.mlu.UntypedStorage._new_with_file(
                f, torch._utils._element_size(z.dtype)
            )
            w_untypedstorage = torch.mlu.UntypedStorage._new_with_file(
                f, torch._utils._element_size(w.dtype)
            )
            x_typedstorage = torch.mlu.DoubleStorage(wrap_storage=x_untypedstorage)
            y_typedstorage = torch.mlu.FloatStorage(wrap_storage=y_untypedstorage)
            z_typedstorage = torch.mlu.ComplexDoubleStorage(
                wrap_storage=z_untypedstorage
            )
            w_typedstorage = torch.mlu.FloatStorage(wrap_storage=w_untypedstorage)
    ...
    def test_untyped_init(self):
        a = torch.mlu.UntypedStorage([1, 2, 3]).device.type
        self.assertTrue(a == "cpu")
        b = torch.mlu.UntypedStorage([1, 2, 3], device="mlu").device.type
    ...
    def test_untyped_get_set(self):
        ...
        b = torch.mlu.UntypedStorage(a.tolist(), device="mlu")
    ...
    def test_storage_exception(self):
        ref_msg = r"torch.mlu.UntypedStorage\(\): Storage device not recognized: cuda"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.mlu.UntypedStorage([1, 2, 3], device="cuda")

        ref_msg = r"Expected a sequence type, but got int"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.mlu.UntypedStorage(sequence=123)
    ...
    # @unittest.skip("not test")
    @testinfo()
    def test_storage_64bit_raise_warn(self):
        a = torch.tensor([3, 4], dtype=torch.double, device="mlu")
        warning_msg = "Because 64 bit data were casted to 32 bit data on MLU memory, "
