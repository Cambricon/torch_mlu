...
class TestAmp(TestCase):
    ...
    def _run_autocast(
        ...
        self.assertFalse(torch.is_autocast_enabled("mlu"))
        with torch.mlu.amp.autocast():
            self.assertTrue(torch.is_autocast_enabled("mlu"))
            self.assertTrue(torch.mlu.is_autocast_enabled())
            self.assertEqual(torch.mlu.get_autocast_dtype(), torch.float16)
            self.assertEqual(torch.get_autocast_dtype("mlu"), torch.float16)
