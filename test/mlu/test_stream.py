...
class TestStream(TestCase):
    ...
    def test_generic_stream_event(self):
        stream = torch.Stream("mlu")
        self.assertEqual(stream.device_index, torch.mlu.current_device())
        mlu_stream = torch.mlu.Stream(
            stream_id=stream.stream_id,
            device_index=stream.device_index,
            device_type=stream.device_type,
        )
        self.assertEqual(stream.stream_id, mlu_stream.stream_id)
        self.assertNotEqual(stream.stream_id, torch.mlu.current_stream().stream_id)

        event1 = torch.Event("mlu", enable_timing=True)
        event2 = torch.Event("mlu", enable_timing=True)
        self.assertEqual(event1.event_id, 0)
        a = torch.randn(1000)
        b = torch.randn(1000)
        with torch.mlu.stream(mlu_stream):
            a_mlu = a.to("mlu", non_blocking=True)
            b_mlu = b.to("mlu", non_blocking=True)
            self.assertEqual(stream.stream_id, torch.mlu.current_stream().stream_id)
        ...
        self.assertEqual(c_mlu.cpu(), a + b)
