...
class TestCatchCounter(TestCase):
    # @unittest.skip("not test")
    @testinfo()
    def test_counter_file(self):
        gencase_dir = "./gen_case"
        for level in ["L1", "L2", "L3"]:
            os.environ["CATCH_COUNTER_LEVEL"] = level
    ...
    def test_counter_number(self):
        ...
        for counter_range in range_list:
            os.environ["CATCH_COUNTER_START"] = str(counter_range[0])
            os.environ["CATCH_COUNTER_END"] = str(counter_range[1])
            reload(torch_mlu.utils.counter)
            _check_gencase()
    ...
    def test_gencase_warnings(self):
        warining_msg_list = [
            "CATCH_COUNTER_START and CATCH_COUNTER_END should be integers",
            "CATCH_COUNTER_START and CATCH_COUNTER_END should" " all be greater than 0",
            "CATCH_COUNTER_END should be greater than CATCH_COUNTER_START",
            "CATCH_COUNTER_START, CATCH_COUNTER_END, CATCH_COUNTER_LEVEL should be"
            " all set to enable catch counter",
            "CATCH_COUNTER enable failed, CATCH_COUNTER_LEVEL only support"
            " L1, L2, L3, but got L0",
            "CATCH_COUNTER enable failed",
        ]
        warning_value_list = ["1.1", "-1", "3", None, "L0", "1"]
        for warning_msg, warning_value in zip(warining_msg_list, warning_value_list):
            if warning_value == "L0":
                os.environ["CATCH_COUNTER_START"] = "1"
                os.environ["CATCH_COUNTER_LEVEL"] = warning_value
            elif warning_value is None:
                del os.environ["CATCH_COUNTER_START"]
            elif warning_value == "1":
                os.environ["CNNL_GEN_CASE"] = "1"
            else:
                os.environ["CATCH_COUNTER_START"] = warning_value
    ...
    def setUp(self):
        super(TestCatchCounter, self).setUp()
        os.environ["CATCH_COUNTER_START"] = "1"
        os.environ["CATCH_COUNTER_END"] = "2"
        os.environ["CATCH_COUNTER_LEVEL"] = "L1"

    def tearDown(self):
        super(TestCatchCounter, self).tearDown()
        gencase_dir = "./gen_case"
        if os.path.exists(gencase_dir):
            shutil.rmtree(gencase_dir)
