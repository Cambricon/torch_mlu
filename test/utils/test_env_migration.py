...
class TestEnvMigration(TestCase):
    ...
    def find_mlu_env_in_file(self, file_path, patterns):
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            ...
            for line in file:
                ...
                env_set.update(patterns[3].findall(line))  # pattern getenv
                buffer = ""
        return env_set
    ...
    def find_cuda_env(self):
        docs_path = os.path.join(
            self.pytorch_home, "docs", "source", "cuda_environment_variables.rst"
        )
        with open(docs_path, "r", encoding="utf-8") as file:
            docs_content = file.read()

        pattern = re.compile(r"\* - ``([A-Z0-9_]+)``")
        matches = pattern.findall(docs_content)
        cuda_env_list = [
            match for match in matches if not match.startswith("TORCH_NCCL_")
        ]

        print(f"Found {len(cuda_env_list)} CUDA environment variables:")
        for cuda_env_var in cuda_env_list:
            print(cuda_env_var)

        return cuda_env_list
