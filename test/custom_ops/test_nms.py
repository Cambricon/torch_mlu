...
class TestNmsOp(TestCase):
    ...
    def test_nms_case1(self):
        ...
        boxes_mlu = boxes.to("mlu")
        scores_mlu = scores.to("mlu")

        for thresh, gt_index in zip(test_thresh, gt_indices):
            keep_indices = torchvision.ops.nms(boxes_mlu, scores_mlu, thresh)
            keep_indices = np.sort(keep_indices.cpu()).tolist()
            self.assertEqual(keep_indices, gt_index)

    # @unittest.skip("not test")
    @testinfo()
    def test_nms_case2(self):
        ...
        keep_indices = torchvision.ops.nms(boxes.to("mlu"), scores.to("mlu"), 0.5)
        keep_indices = np.sort(keep_indices.cpu())
        self.assertEqual(keep_indices, gt_indices)

    # @unittest.skip("not test")
    @testinfo()
    def test_nms_exception(self):
        boxes = torch.from_numpy(
        ...
        ref_msg = (
            r"^boxes and scores should have same number of elements in dimension 0, "
        )
        ref_msg = ref_msg + r"got 2 and 3$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to("mlu"), scores.to("mlu"), 0.5)

        scores = torch.from_numpy(np.array([[0.5, 0.7]]).astype(np.float32))
        ref_msg = r"^scores should be a 1d tensor, got 2D$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to("mlu"), scores.to("mlu"), 0.5)

        boxes = torch.from_numpy(
            np.array([10, 10, 50, 60, 11, 12, 48, 60])
            .astype(np.float32)
            .reshape(-1, 1, 4)
        )
        ref_msg = r"^boxes should be a 2d tensor, got 3D$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to("mlu"), scores.to("mlu"), 0.5)

        boxes = torch.from_numpy(
            np.array([10, 10, 50, 60, 12, 11, 12, 48, 60, 62])
            .astype(np.float32)
            .reshape(-1, 5)
        )
        ref_msg = r"^boxes should have 4 elements in dimension 1, got 5$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to("mlu"), scores.to("mlu"), 0.5)
        ...
        scores = torch.from_numpy(np.array([0.5, 0.7]).astype(np.float32))
        ref_msg = r"^Expected all tensors to be on the same device, but found at least two devices"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to("mlu"), scores, 0.5)

        scores = torch.from_numpy(np.array([0.5, 0.7]).astype(np.float64))
        ref_msg = r"^dets should have the same type as scores$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to("mlu"), scores.to("mlu"), 0.5)

    # @unittest.skip("not test")
    @testinfo()
    def test_nms_dtype(self):
        boxes = torch.from_numpy(
            np.array([10, 10, 50, 60, 11, 12, 48, 60]).astype(np.float32).reshape(-1, 4)
        )
        scores = torch.from_numpy(np.array([0.5, 0.8]).astype(np.float32))
        type_list = [
            torch.double,
            torch.float,
            torch.half,
        ...
        for dtype in type_list:
            ...
            if dtype not in supported_type_list:
                ref_msg = r"^\"MLU nms\" not implemented for "
                with self.assertRaisesRegex(RuntimeError, ref_msg):
                    torchvision.ops.nms(boxes.to("mlu"), scores.to("mlu"), 0.5)

    @testinfo()
    def test_nms_with_amp(self):
        inputs = (
            np.array(
                [
                    10,
        ...
        with torch.amp.autocast(device_type="mlu"):
            for thresh, gt_index in zip(test_thresh, gt_indices):
                keep_indices = torchvision.ops.nms(boxes_mlu, scores_mlu.half(), thresh)
                keep_indices = np.sort(keep_indices.cpu()).tolist()
                self.assertEqual(keep_indices, gt_index)
